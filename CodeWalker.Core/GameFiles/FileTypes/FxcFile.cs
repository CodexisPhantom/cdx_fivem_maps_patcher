...
namespace CodeWalker.GameFiles
{
    ...
    public class FxcFile : PackedFile
    {
        ...
        public void Load(byte[] data, RpfFileEntry entry)
        {
            ...
            if (ppCount > 0)
            {
                List<FxcPresetParam> pparams = new List<FxcPresetParam>();
            ...
            List<FxcShaderGroup> groups = new List<FxcShaderGroup>();
            List<FxcShader> shaders = new List<FxcShader>();
            for (int i = 0; i < 6; i++)
            {
                FxcShaderGroup group = new FxcShaderGroup();
                group.Read(br, i);
                groups.Add(group);
                if (group.Shaders != null) shaders.AddRange(group.Shaders);
            }
            ...
            if (cbCount1 > 0)
            {
                List<FxcCBuffer> cbuffers1 = new List<FxcCBuffer>();
            ...
            if (varCount1 > 0)
            {
                List<FxcVariable> vars1 = new List<FxcVariable>(); //cbuffer contents/vars
                for (int i = 0; i < varCount1; i++)
                {
                    FxcVariable vari = new FxcVariable();
                    vari.Read(br);
                    vars1.Add(vari);
                    if (CBufferDict.TryGetValue(vari.CBufferName, out FxcCBuffer cbtmp))
            ...
            if (cbCount2 > 0)
            {
                List<FxcCBuffer> cbuffers2 = new List<FxcCBuffer>(); //more cbuffers..
            ...
            if (varCount2 > 0)
            {
                List<FxcVariable> vars2 = new List<FxcVariable>();
            ...
            if (techCount > 0)
            {
                List<FxcTechnique> techniques = new List<FxcTechnique>();
            ...
            foreach (FxcCBuffer cbuf in CBufferDict.Values) cbuf.ConsolidateVariables();
        ...
        public byte[] Save()
        {
            ...
            byte ppCount = (byte)(PresetParams?.Length ?? 0);
            w.Write(ppCount);
            for (int i = 0; i < ppCount; i++) PresetParams[i].Write(w);
            ...
            for (int i = 0; i < 6; i++)
            {
                FxcShaderGroup group = ShaderGroups[i];
                group.Write(w, i);
            }


            byte cbCount1 = (byte)(CBuffers1?.Length ?? 0);
            w.Write(cbCount1);
            for (int i = 0; i < cbCount1; i++) CBuffers1[i].Write(w);

            byte varCount1 = (byte)(Variables1?.Length ?? 0);
            w.Write(varCount1);
            for (int i = 0; i < varCount1; i++) Variables1[i].Write(w);

            byte cbCount2 = (byte)(CBuffers2?.Length ?? 0);
            w.Write(cbCount2);
            for (int i = 0; i < cbCount2; i++) CBuffers2[i].Write(w);

            byte varCount2 = (byte)(Variables2?.Length ?? 0);
            w.Write(varCount2);
            for (int i = 0; i < varCount2; i++) Variables2[i].Write(w);

            byte techCount = (byte)(Techniques?.Length ?? 0);
            w.Write(techCount);
            for (int i = 0; i < techCount; i++) Techniques[i].Write(w);
        ...
        public void WriteXml(StringBuilder sb, int indent, string csofolder)
        {
            ...
            if (PresetParams != null) FxcXml.WriteItemArray(sb, PresetParams, indent, "PresetParams");
            ...
            {
                FxcShaderGroup group = ShaderGroups[i];
                string typen = group.Type + "s";
                string tagn = typen;
                if (group.OffsetBy1) tagn += " OffsetBy1=\"" + group.OffsetBy1 + "\"";
            ...
            if (CBuffers1 != null) FxcXml.WriteItemArray(sb, CBuffers1, indent, "CBuffers1");
            if (Variables1 != null) FxcXml.WriteItemArray(sb, Variables1, indent, "Variables1");
            if (CBuffers2 != null) FxcXml.WriteItemArray(sb, CBuffers2, indent, "CBuffers2");
            if (Variables2 != null) FxcXml.WriteItemArray(sb, Variables2, indent, "Variables2");
            if (Techniques != null) FxcXml.WriteItemArray(sb, Techniques, indent, "Techniques");
        }
        ...
        public void ReadXml(XmlNode node, string csofolder)
        {
            ...
            XmlNode snode = node.SelectSingleNode("Shaders");
            if (snode != null)
            {
                List<FxcShader> shaders = new List<FxcShader>();
                ...
                {
                    string gname = type + "s";
                    XmlNode gnode = snode.SelectSingleNode(gname);
                ...
                FxcShaderGroup[] groups = new FxcShaderGroup[6];
                for (int i = 0; i < 6; i++)
                {
                    FxcShaderGroup group = getShaderGroup((FxcShaderType)i);
                    groups[i] = group;
                    if (group.Shaders != null) shaders.AddRange(group.Shaders);
                }
            ...
            if (Techniques != null)
                foreach (FxcTechnique t in Techniques)
        ...
        public string GetMetaString()
        {
            ...
            sb.AppendLine("Vertex type: " + ((uint)VertexType));
            sb.AppendLine();

            if (PresetParams != null)
            {
                sb.AppendLine("Header");
                foreach (FxcPresetParam ext in PresetParams) sb.AppendLine("  " + ext);
                sb.AppendLine();
            }
            ...
            if (ShaderGroups != null)
            {
                ...
                foreach (FxcShaderGroup chunk in ShaderGroups) sb.AppendLine("  " + chunk);
                sb.AppendLine();
            }

            if (Shaders != null)
            {
                sb.AppendLine("Shaders (" + Shaders.Length + ")");
                foreach (FxcShader shader in Shaders)
                {
                    sb.AppendLine("  " + shader.Name);
                    if (shader.Variables != null && shader.Variables.Length > 0)
                    {
                        sb.AppendLine("    (Params)");
                        foreach (string parm in shader.Variables) sb.AppendLine("      " + parm);
                    }

                    if (shader.Buffers != null && shader.Buffers.Length > 0)
                    {
                        sb.AppendLine("    (Buffers)");
                        foreach (FxcShaderBufferRef ext in shader.Buffers) sb.AppendLine("      " + ext);
                    }
                }
            ...
            if (CBuffers1 != null)
            {
                sb.AppendLine("CBuffers 1 (" + CBuffers1.Length + ")");
                foreach (FxcCBuffer p in CBuffers1) sb.AppendLine("  " + p);
                sb.AppendLine();
            }

            if (Variables1 != null)
            {
                sb.AppendLine("Variables 1 (" + Variables1.Length + ")");
                foreach (FxcVariable p in Variables1)
                {
                    sb.AppendLine("  " + p);
                    if (p.Params != null && p.Params.Length > 0)
                    {
                        sb.AppendLine("    (Params)");
                        foreach (FxcVariableParam c in p.Params) sb.AppendLine("      " + c);
                    }

                    if (p.ValuesF != null && p.ValuesF.Length > 0)
                    {
                        sb.AppendLine("    (Values)");
                        foreach (float d in p.ValuesF) sb.AppendLine("      " + d);
                    }
                }
            ...
            if (CBuffers2 != null)
            {
                sb.AppendLine("CBuffers 2 (" + CBuffers2.Length + ")");
                foreach (FxcCBuffer p in CBuffers2) sb.AppendLine("  " + p);
                sb.AppendLine();
            }

            if (Variables2 != null)
            {
                sb.AppendLine("Variables 2 (" + Variables2.Length + ")");
                foreach (FxcVariable p in Variables2)
                {
                    sb.AppendLine("  " + p);
                    if (p.Params != null && p.Params.Length > 0)
                    {
            ...
            if (Techniques != null)
            {
                sb.AppendLine("Techniques (" + Techniques.Length + ")");
                foreach (FxcTechnique t in Techniques)
                {
                    ...
                    if (t.PassCount > 0 && t.Passes != null)
                    {
                        ...
                        foreach (FxcPass p in t.Passes)
                        {
                            sb.AppendLine("      " + p);
                            if (p.ParamCount > 0 && p.Params != null)
                                //sb.AppendLine("    Params");
                                foreach (FxcPassParam v in p.Params)
                                    sb.AppendLine("         " + v);
                        }
                    }
                }
        ...
        {
            int i = id - 1;
            if (i < 0 || VertexShaders == null || i >= VertexShaders.Length) return null;
        ...
        {
            int i = id - 1;
            if (i < 0 || PixelShaders == null || i >= PixelShaders.Length) return null;
        ...
        {
            int i = id - 1;
            if (i < 0 || ComputeShaders == null || i >= ComputeShaders.Length) return null;
        ...
        {
            int i = id - 1;
            if (i < 0 || DomainShaders == null || i >= DomainShaders.Length) return null;
        ...
        {
            int i = id - 1;
            if (i < 0 || GeometryShaders == null || i >= GeometryShaders.Length) return null;
        ...
        {
            int i = id - 1;
            if (i < 0 || HullShaders == null || i >= HullShaders.Length) return null;
    ...
    [TC(typeof(EXP))]
    public class FxcPresetParam : IMetaXmlItem
    {
    ...
    public class FxcShaderGroup
    {
        ...
        public void Write(BinaryWriter bw, int gindex)
        {
            ...
            if (OffsetBy1) bw.Write((byte)0);
            bw.Write(ShaderCount);
            FxcFile.WriteString(bw, Name);
            bw.Write(Unk1Byte);
            bw.Write(Unk2Byte);
            bw.Write(Unk3Uint);
            for (int i = 1; i < ShaderCount; i++) Shaders[i - 1].Write(bw, gindex);
        }

        public void WriteXml(StringBuilder sb, int indent, string csofolder)
        {
            int sc = Shaders?.Length ?? 0;
            int ci = indent + 1;
            int si = ci + 1;
            for (int i = 0; i < sc; i++)
            {
                string typen = "Item"; // Index=\"" + i.ToString() + "\"";
        ...
        public void ReadXml(XmlNode node, string csofolder)
        {
            ...
            if (inodes?.Count > 0)
            {
                List<FxcShader> slist = new List<FxcShader>();
                foreach (XmlNode inode in inodes)
                {
                    FxcShader s = new FxcShader();
                    s.Type = Type;
        ...
        public override string ToString()
        {
            return Name + ": " + Type + ": " + ShaderCount + ": " + Unk1Byte + ": " + Unk2Byte + ": " + Unk3Uint;
        }
    }
    ...
    public class FxcShader
    {
        ...
        public void Read(BinaryReader br, int gindex)
        {
            ...
            if (bufferCount > 0)
            {
                List<FxcShaderBufferRef> buffers = new List<FxcShaderBufferRef>();
            ...
            {
                byte exbyte = br.ReadByte(); //not sure what this is used for...
                if (exbyte != 0)
                {
                } //no hit
            }
        ...
        public void Write(BinaryWriter bw, int gindex)
        {
            ...
            byte bufferCount = (byte)(Buffers?.Length ?? 0);
            bw.Write(bufferCount);
            for (int i = 0; i < bufferCount; i++) Buffers[i].Write(bw);

            if (Type == FxcShaderType.GeometryShader) //GS seems to be slightly different format??
                bw.Write((byte)0); //why is this here..? crazy GS

            uint dataLength = (uint)(ByteCode?.Length ?? 0);
            bw.Write(dataLength);
            if (dataLength > 0)
            {
                bw.Write(ByteCode);
        ...
        public void WriteXml(StringBuilder sb, int indent, string csofolder)
        {
            string fname = (Name?.Replace("/", "")?.Replace("\\", "") ?? "NameError") + ".cso";
            FxcXml.StringTag(sb, indent, "Name", FxcXml.XmlEscape(Name));
            FxcXml.StringTag(sb, indent, "File", FxcXml.XmlEscape(fname));
            if (OffsetBy1) FxcXml.ValueTag(sb, indent, "OffsetBy1", OffsetBy1.ToString());
            switch (Type)
            {
                case FxcShaderType.VertexShader:
                case FxcShaderType.PixelShader:
            ...
            if (Buffers != null) FxcXml.WriteItemArray(sb, Buffers, indent, "Buffers");
            if (ByteCode != null)
            {
                bool export = !string.IsNullOrEmpty(csofolder);
                if (export)
                    try
                    {
                        if (!Directory.Exists(csofolder)) Directory.CreateDirectory(csofolder);
                        string filepath = Path.Combine(csofolder, fname);
        ...
        public void ReadXml(XmlNode node, string csofolder)
        {
            ...
            XmlNode vnode = node.SelectSingleNode("Variables");
            if (vnode != null)
            {
                ...
                if (inodes?.Count > 0)
                {
                    ...
                    foreach (XmlNode inode in inodes) slist.Add(inode.InnerText);
            ...
            string filename = Xml.GetChildInnerText(node, "File")?.Replace("/", "")?.Replace("\\", "");
            if (!string.IsNullOrEmpty(filename) && !string.IsNullOrEmpty(csofolder))
            {
                string filepath = Path.Combine(csofolder, filename);
    ...
    [TC(typeof(EXP))]
    public class FxcShaderBufferRef : IMetaXmlItem
    {
        public string Name { get; set; }
    ...
    [TC(typeof(EXP))]
    public class FxcCBuffer : IMetaXmlItem
    {
        ...
        public uint NameHash => JenkHash.GenHash(Name?.ToLowerInvariant());
        ...
        public void Read(BinaryReader br)
        {
            ...
            Name = FxcFile
                .ReadString(
                    br); // <fxc name> _locals   //"rage_matrices", "misc_globals", "lighting_globals", "more_stuff"
        ...
        public override string ToString()
        {
            return Name + ": " + Size + ", " + SlotVS + ", " + SlotPS + ", " + SlotCS + ", " + SlotDS + ", " + SlotGS +
                   ", " + SlotHS;
        }
    ...
    [TC(typeof(EXP))]
    public class FxcVariable : IMetaXmlItem
    {
            ...
            Variant
        {
            get;
            set;
        } //255,0,1,2,3,4,5,7,9  seems to be used when multiple variables at same offset in the buffer
        ...
        public void WriteXml(StringBuilder sb, int indent)
        {
            ...
            if (Params != null) FxcXml.WriteItemArray(sb, Params, indent, "Params");
            if (ValuesF != null) FxcXml.WriteRawArray(sb, ValuesF, indent, "Values", "", FloatUtil.ToString);
            if (ValuesU != null) FxcXml.WriteRawArray(sb, ValuesU, indent, "Values", "");
        }
        ...
        public void Read(BinaryReader br)
        {
            ...
            if (ValueCount > 0)
            {
                if (UseUIntValues)
                {
                    ...
                    for (int i = 0; i < ValueCount; i++) ValuesU[i] = br.ReadUInt32();
                }
                else
                {
                    ValuesF = new float[ValueCount];
                    for (int i = 0; i < ValueCount; i++) ValuesF[i] = br.ReadSingle();
                }
            }
        ...
        public void Write(BinaryWriter bw)
        {
            ...
            bw.Write(ParamCount);
            for (int i = 0; i < ParamCount; i++) Params[i].Write(bw);

            if (UseUIntValues)
            {
                ValueCount = (byte)(ValuesU?.Length ?? 0);
                bw.Write(ValueCount);
                for (int i = 0; i < ValueCount; i++) bw.Write(ValuesU[i]);
            }
            else
            {
                ValueCount = (byte)(ValuesF?.Length ?? 0);
                bw.Write(ValueCount);
                for (int i = 0; i < ValueCount; i++) bw.Write(ValuesF[i]);
            }
        }
        ...
        public override string ToString()
        {
            string pstr = ParamCount > 0 ? ", " + ParamCount + " params" : "";
            string vstr = ValueCount > 0 ? ", " + ValueCount + " values" : "";
            return Type + ", " + Count + ", " + Slot + ", " + Group + ", " + Offset + ", " + Variant + ", " +
                   CBufferName + ", " + Name1 + ", " + Name2 + pstr + vstr;
        }
    }
    ...
    [TC(typeof(EXP))]
    public class FxcVariableParam : IMetaXmlItem
    {
        public string Name { get; set; }
    ...
    [TC(typeof(EXP))]
    public class FxcTechnique : IMetaXmlItem
    {
        public string Name { get; set; }
        ...
        public void Write(BinaryWriter bw)
        {
            ...
            bw.Write(PassCount);
            for (int i = 0; i < PassCount; i++) Passes[i].Write(bw);
        }

        public void GetNamesFromIndices(FxcFile fxc)
        {
            if (Passes == null) return;
            foreach (FxcPass pass in Passes) pass.GetNamesFromIndices(fxc);
        }

        public void GetIndicesFromNames(FxcFile fxc)
        {
            if (Passes == null) return;
            foreach (FxcPass pass in Passes) pass.GetIndicesFromNames(fxc);
        }

        public override string ToString()
        {
            return Name + " (" + PassCount + " pass" + (PassCount != 1 ? "es" : "") + ")";
        }
    }

    [TC(typeof(EXP))]
    public class FxcPass : IMetaXmlItem
    {
        ...
        public override string ToString()
        {
            return VS + ", " + PS + ", " + CS + ", " + DS + ", " + GS + ", " + HS;
        }
    }

    [TC(typeof(EXP))]
    public class FxcPassParam : IMetaXmlItem
    {

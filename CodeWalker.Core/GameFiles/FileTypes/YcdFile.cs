...
");
            ...
            foreach (AnimationBoneId list in crAnim.BoneIds.data_items)
            {
                ...
                foreach (Sequence seq in crAnim.Sequences.data_items)
                {
                    AnimChannel[] channels = seq.Sequences[i].Channels;
                    ...
                    {
                        string chanType = ChannelType(channels[0]);
                        string chanData = ChannelData(channels[0]);

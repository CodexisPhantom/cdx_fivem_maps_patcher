...
namespace CodeWalker.GameFiles
{
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YndFile : GameFile, PackedFile, BasePathData
    {
            ...
            if (Nodes != null)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (node.Links != null)
                    {
                        ...
                        {
                            YndLink nlink = node.Links[l];
                            NodeLink rlink = nlink.RawData;
                            if (nlink.Node2 != null)
                            {
                    ...
                    if ((node.HasJunction) && (node.Junction != null))
                    {
                        ...
                        byte[] heightmap = nj.Heightmap.GetBytes();
                        nj._RawData.HeightmapPtr = (ushort)heightmapoff;
                        NodeJunctionRef jref = nj.RefData;
        ...
        public void InitNodesFromDictionary()
        {
            if (NodeDictionary != null)
            {
                ...
                {
                    Node[] nodes = NodeDictionary.Nodes;
                    Nodes = new YndNode[nodes.Length];
                    for (int i = 0; i < nodes.Length; i++)
                    {
                        YndNode n = new YndNode();
                        n.Init(this, nodes[i]);
                        Nodes[i] = n;
                        if (n.NodeID != i)
        ...
        public void RecalculateNodeIndices()
        {
            ...
            YndNode[] vehicleNodes = Nodes.Where(n => !n.IsPedNode).OrderBy(n => n.NodeID).ToArray();
            YndNode[] pedNodes = Nodes.Where(n => n.IsPedNode).OrderBy(n => n.NodeID).ToArray();
        ...
        public bool RemoveLinksForNode(YndNode node)
        {
            ...
            List<YndLink> rmLinks = new List<YndLink>();

            foreach (YndNode n in Nodes)
            {
                IEnumerable<YndLink> nodeRmLinks = n.Links.Where(l =>
        ...
        private void UpdateLinkTriangleVertices()
        {
            ...
            if ((Links != null) && (Nodes != null))
            {
                foreach (YndNode node in Nodes)
                {
                    ...
                    {
                        Vector3 p0 = link.Node1?.Position ?? Vector3.Zero;
                        Vector3 p1 = link.Node2?.Position ?? Vector3.Zero;
                        Vector3 dir = link.GetDirection();
                        ...
                        uint c = (uint)link.GetColour().ToRgba();
                        v0.Colour = c;
                        v1.Colour = c;
                        v2.Colour = c;
                        v3.Colour = c;
        ...
        private void UpdateJunctionTriangleVertices(YndNode[] selectedNodes)
        {
            ...
            foreach (YndNode node in selectedNodes)
            {
                if (node.Ynd != this) continue;
                if (node.Junction == null) continue;
                YndJunction j = node.Junction;
                YndJunctionHeightmap d = j.Heightmap;
                if (d == null) continue;
                ...
                {
                    YndJunctionHeightmapRow row0 = d.Rows[y - 1];
                    YndJunctionHeightmapRow row1 = d.Rows[y];
                    float offy = y * 2.0f;

                    for (int x = 1; x < d.CountX; x++) //values progress along the X axis.
                    {
                        float val0 = row0.Values[x - 1] / 255.0f;
                        float val1 = row0.Values[x] / 255.0f;
                        float val2 = row1.Values[x - 1] / 255.0f;
                        float val3 = row1.Values[x] / 255.0f;
                        float offx = x * 2.0f;
            ...
            if (verts.Count > 0)
            {
                ...
                {
                    int nvc = vertsarr.Length;
                    int tvc = TriangleVerts.Length;
                    EditorVertex[] newTriangles = new EditorVertex[tvc + nvc];
    ...
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YndNode : BasePathNode
    {
        ...
        {
            YndLink link = Links?.FirstOrDefault(l => l.LaneCountBackward > 0);
            if (link is null)
            {
                HeuristicValue = 0;
                return;
            ...
            YndNode partner = link.Node1 == this
                ? link.Node2
                : link.Node1;

            FlagsByte length = link.LinkLength;
        ...
        {
            List<YndFile> affectedFilesList = new List<YndFile>();
        ...
        public void SetYndNodePosition(Space space, Vector3 newPosition, out YndFile[] affectedFiles)
        {
            ...
            SpaceNodeGridCell expectedArea = space.NodeGrid.GetCellForPosition(newPosition);

            if (AreaID != expectedArea.ID)
            {
                YndFile nodeYnd = space.NodeGrid.GetCell(AreaID).Ynd;
                YndFile newYnd = expectedArea.Ynd;
                if (newYnd == null)
                {
        ...
        public void GenerateYndNodeJunctionHeightMap(Space space)
        {
            ...
            float minZ = junc.MinZ / 32f;
            float xStart = junc.PositionX / 4f;
            float yStart = junc.PositionY / 4f;
            byte sizeX = junc._RawData.HeightmapDimX;
            ...
            for (int y = 0; y < sizeY; y++)
            {
                ...
                for (int x = 0; x < sizeX; x++)
                {
                    ...
                    float height = Math.Min(Math.Max(result.Position.Z, minZ), maxZ);
                    byte actualDist = (byte)((height - minZ) / maxDist * 255);
    ...
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YndLink
    {
        ...
        public Color4 GetColour()
        {
            ...
            Color4 c = new Color4(0.0f, 0.0f, 0.0f, 0.5f);
        ...
        {
            Vector3 p0 = Node1?.Position ?? Vector3.Zero;
            Vector3 p1 = Node2?.Position ?? Vector3.Zero;
            Vector3 diff = p1 - p0;
    ...
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YndJunctionHeightmap
    {
        ...
        {
            YndJunctionHeightmapRow[] nrows = new YndJunctionHeightmapRow[cy];
            for (int y = 0; y < cy; y++)
            {

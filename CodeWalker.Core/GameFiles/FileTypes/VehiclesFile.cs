...
namespace CodeWalker.GameFiles
{
    ...
    public class VehicleInitData
    {
        ...
        public void Load(XmlNode node)
        {
            ...
            XmlNode dnode = node.SelectSingleNode("drivers");
            if (dnode != null)
            {
                XmlNodeList items = dnode.SelectNodes("Item");
                if (items.Count > 0)
                {
        ...
        {
            XmlNode cnode = node.SelectSingleNode(childName);
        ...
        private string[] GetStringArray(XmlNode node, string childName, char delimiter)
        {
            ...
            string[] ldarr = ldastr?.Split(delimiter);

...
namespace CodeWalker.GameFiles
{
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YnvFile : GameFile, PackedFile, BasePathData
    {
        ...
        public void InitFromNav()
        {
            ...
            {
                List<NavMeshVertex> verts = Nav.Vertices.GetFullList();
                Vertices = new List<Vector3>(verts.Count);
                for (int i = 0; i < verts.Count; i++)
                {
                    Vector3 ov = verts[i].ToVector3();
        ...
        private void BuildStructs()
        {
            ...
            List<NavMeshVertex> vertlist = new List<NavMeshVertex>();
            List<ushort> indslist = new List<ushort>();
            List<NavMeshEdge> edgelist = new List<NavMeshEdge>();
            List<NavMeshPoly> polylist = new List<NavMeshPoly>();
            List<NavMeshPortal> portallist = new List<NavMeshPortal>();
            List<ushort> portallinks = new List<ushort>();

            Dictionary<Vector3, ushort> vertdict = new Dictionary<Vector3, ushort>();
            Dictionary<uint, uint> areadict = new Dictionary<uint, uint>();
            List<uint> arealist = new List<uint>();
            uint areaid = Nav.AreaID;
            ...
            if (Portals != null)
            {
                for (int i = 0; i < Portals.Count; i++)
                {
                    ...
                    NavMeshPortal pdata = portal.RawData;
            ...
            if (Points != null) //points will be built into the sector tree
            {
                for (int i = 0; i < Points.Count; i++)
                {
                    ...
                    NavMeshPoint pdata = point.RawData;
            ...
            {
                NavMeshListPart<NavMeshPoly> listpart = Nav.Polys.ListParts[i];
            ...
            uint pointindex = 0;
            bool[] pointflags = new bool[Points?.Count ?? 0];
        ...
        public void UpdateAllNodePositions()
        {
            ...
            v.Colour = 0xFF0000FF;
            List<EditorVertex> lv = new List<EditorVertex>();
            List<Vector4> nv = new List<Vector4>();
    ...
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YnvPoly : IMetaXmlItem
    {
        ...
        public Color4 GetColour()
        {
            ...
            byte u1 = _RawData.PortalLinkCount;
    ...
    [TypeConverter(typeof(ExpandableObjectConverter))] public class YnvPortal : BasePathNode, IMetaXmlItem
    {
        ...
        public void SetPosition(Vector3 pos)
        {
            Vector3 delta = pos - PositionFrom;
            PositionFrom = pos;
            PositionTo += delta;

...
namespace CodeWalker.GameFiles
{
    ...
    public class WatermapFile : GameFile, PackedFile
    {
        ...
        private void Read(DataReader r)
        {
            ...
            int flagoff = 0; //assign extra colours out of the main array
            ...
            {
                WaterItemRef ir = CompWatermapRefs[i];
                switch (ir.Type)
                {
            ...
            List<WaterItemRef> reflist = new List<WaterItemRef>();
            for (int y = 0; y < Height; y++)
            {
                CompHeader ch = CompHeaders[y];
                for (int i = 0; i < ch.Count; i++)
                {
                    ...
                    if (n >= 0)
                    {
                        WaterItemRef h = CompWatermapRefs[n];
                        reflist.Add(h);
                        short cn = n;

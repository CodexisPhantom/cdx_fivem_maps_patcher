...
namespace CodeWalker.GameFiles
{
    ...
    public class ResourceAnalyzer
    {
        public ResourceAnalyzer(ResourceDataReader reader)
        {
            ...
            bool gen9 = reader.IsGen9;

            List<ResourceAnalyzerItem> dlist = new List<ResourceAnalyzerItem>();
            foreach (KeyValuePair<long, IResourceBlock> kvp in reader.blockPool)
            {
                ResourceAnalyzerItem item = new ResourceAnalyzerItem();
            ...
            foreach (KeyValuePair<long, object> kvp in reader.arrayPool)
            {
                ResourceAnalyzerItem item = new ResourceAnalyzerItem();
                item.Position = kvp.Key;
                ...
                if (item.Array != null)
                {
                    Type typ = item.Array.GetType().GetElementType();
                    int siz = Marshal.SizeOf(typ);
                    item.Length = item.Array.Length * siz;
                }
                else
                {
                    item.String = kvp.Value as string;
                    if (item.String != null) item.Length = item.String.Length + 1;
                }
            ...
            List<ResourceAnalyzerItem> dlist2 = new List<ResourceAnalyzerItem>();
            long pos = 0;
            bool gfx = false;
            foreach (ResourceAnalyzerItem item in dlist)
            {
                ...
                if (item.Offset > pos)
                {
                    ResourceAnalyzerItem gap = new ResourceAnalyzerItem();
                    gap.Position = pos;
                ...
                if (item.Offset == pos)
                {
                    ...
                    if (item.String == null)
                        if (pos % 16 != 0)
                            pos += 16 - pos % 16; //ignore alignment paddings
                }
                else
                {
                    item.Overlapping = true;
                    dlist2.Add(item);
                    long pos2 = item.Offset + item.Length;
        ...
        public class ResourceAnalyzerItem
        {
            ...
            public long Offset => Position & 0xFFFFFFF;
            ...
            public override string ToString()
            {
                ...
                {
                    type = Array.GetType().Name + " (" + Array.Length + ")";
                }
                else if (String != null)
                {
                    type = "string";
                ...
                string valstr = string.IsNullOrEmpty(val) ? "" : " - " + val;
                return Offset + " - " + Length + " - " + type + valstr + (Overlapping ? "   (embedded)" : "");
            }
        }
    }
}
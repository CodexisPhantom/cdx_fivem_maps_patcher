...
namespace CodeWalker.GameFiles
{
    public class XmlPso
    {
        ...
        private static byte[] Traverse(XmlNode node, PsoBuilder pb, MetaName type = 0, bool isRoot = false)
        {
            ...
            if (infos != null)
            {
                    ...
                    switch (entry.Type)
                    {
                        ...
                        case PsoDataType.Structure:
                        {
                            ...
                            if (stype == 0)
                            {
                                ...
                                if (!string.IsNullOrEmpty(stypestr)) stype = (MetaName)(uint)GetHash(stypestr);
                            }
                        ...
                        case PsoDataType.Enum:
                        {
                            ...
                            switch (entry.Unk_5h)
                            {
                                case 0: //int enum
                                    ...
                                    break;
                                case 1: //short enum?
                                    short sval = (short)GetEnumInt((MetaName)entry.ReferenceKey, cnode.InnerText,
                                        entry.Type);
                                    Write(sval, data, entry.DataOffset);
                                    break;
                                case 2: //byte enum
                                    byte bval = (byte)GetEnumInt((MetaName)entry.ReferenceKey, cnode.InnerText,
                                        entry.Type);
                        ...
                        case PsoDataType.Flags:
                        {
                            ...
                            if (flagsInfo != null) pb.AddEnumInfo(flagsInfo.IndexInfo.NameHash);
                ...
                if (isRoot) pb.RootPointer = pb.AddItem(type, data);
        ...
        private static void TraverseMap(XmlNode node, PsoBuilder pb, PsoStructureEntryInfo entry,
            PsoStructureInfo infos, byte[] data, PsoArrayResults arrayResults)
        {
            ...
            arrayResults.Structures[entry.DataOffset + 8] =
                pb.AddItemArrayPtr(xName, nodesData.ToArray()); //pb.AddPointerArray(nodeptrs);
        }

        private static void TraverseArray(XmlNode node, PsoBuilder pb, PsoStructureEntryInfo entry,
            PsoStructureEntryInfo arrEntry, PsoArrayResults results, byte[] data, PsoStructureInfo structInfo)
        {
            ...
            switch (arrEntry.Type)
            {
                case PsoDataType.Structure:
                {
                    ...
                    {
                        if (arrEntry.ReferenceKey != 0)
                            results.Structures[offset] =
                                TraverseArrayStructure(node, pb, (MetaName)arrEntry.ReferenceKey);
                        else
                ...
                case PsoDataType.String:
                {
                    switch (arrEntry.Unk_5h)
                    {
                        ...
                        case 2: //string array  (array of pointers)
                            ...
                            PsoBuilderPointer aptr = cnt > 0
                                ? pb.AddItemArray((MetaName)MetaTypeName.PsoPOINTER, ptrs)
                                : new PsoBuilderPointer();
                            results.Structures[offset] = cnt > 0
                                ? new Array_Structure(aptr.Pointer, aptr.Length)
                                : new Array_Structure();
                            break;
                        case 3: //char array array  (array of CharPointer)
                            ...
                            results.Structures[offset] = cnt2 > 0
                                ? new Array_Structure(aptr2.Pointer, aptr2.Length)
                                : new Array_Structure();
                            break;
                    }
                ...
                case PsoDataType.Enum:
                {
                    ...
                    if (arrEntry.ReferenceKey != 0)
                    {
                        ...
                        {
                            MetaName enumname =
                                (MetaName)MetaTypes.SwapBytes(hashes[i]); //yeah swap it back to little endian..!
                            PsoEnumEntryInfo enuminf = _infos.FindEntryByName(enumname);
                            if (enuminf != null) values[i] = MetaTypes.SwapBytes((uint)enuminf.EntryKey);
                            //error?
                        }
        ...
        private static void TraverseStringRaw(string str, PsoBuilder pb, PsoStructureEntryInfo entry, byte[] data)
        {
            switch (entry.Unk_5h)
            {
                ...
                case 8: //hash with STRF entry?
                    ...
                    if (entry.Unk_5h == 8) pb.AddStringToSTRF(str);
                    break;
            }
        }
        ...
        private static XmlNode GetEntryNode(XmlNodeList nodes, MetaName name)
        {
            foreach (XmlNode node in nodes)
            {
                ...
                if (GetHash(node.Name) == (uint)name) return node;
            }
        ...
        private static int GetEnumInt(MetaName type, string enumString, PsoDataType dataType)
        {
...

            {
                ...
                for (int j = 0; j < infos.Entries.Length; j++)
                {
                    ...
                    if (entry.EntryNameHash == enumName) return entry.EntryKey;
                }
            }
    ...
    internal struct PsoArrayResults
    {

...
namespace CodeWalker.GameFiles
{
    public class XmlMeta
    {
        ...
        public static MetaFormat GetXMLFormat(string fnamel, out int trimlength)
        {
            ...
            if (fnamel.EndsWith(".rel.xml")) mformat = MetaFormat.AudioRel;
            if (fnamel.EndsWith(".ynd.xml")) mformat = MetaFormat.Ynd;
            if (fnamel.EndsWith(".ynv.xml")) mformat = MetaFormat.Ynv;
            if (fnamel.EndsWith(".ycd.xml")) mformat = MetaFormat.Ycd;
            if (fnamel.EndsWith(".ybn.xml")) mformat = MetaFormat.Ybn;
            if (fnamel.EndsWith(".ytd.xml")) mformat = MetaFormat.Ytd;
            if (fnamel.EndsWith(".ydr.xml")) mformat = MetaFormat.Ydr;
            if (fnamel.EndsWith(".ydd.xml")) mformat = MetaFormat.Ydd;
            if (fnamel.EndsWith(".yft.xml")) mformat = MetaFormat.Yft;
            if (fnamel.EndsWith(".ypt.xml")) mformat = MetaFormat.Ypt;
            if (fnamel.EndsWith(".yld.xml")) mformat = MetaFormat.Yld;
            if (fnamel.EndsWith(".yed.xml")) mformat = MetaFormat.Yed;
            if (fnamel.EndsWith(".ywr.xml")) mformat = MetaFormat.Ywr;
            if (fnamel.EndsWith(".yvr.xml")) mformat = MetaFormat.Yvr;
            if (fnamel.EndsWith(".awc.xml")) mformat = MetaFormat.Awc;
            if (fnamel.EndsWith(".fxc.xml")) mformat = MetaFormat.Fxc;
            if (fnamel.EndsWith("cache_y.dat.xml")) mformat = MetaFormat.CacheFile;
            if (fnamel.EndsWith(".dat.xml") && fnamel.StartsWith("heightmap")) mformat = MetaFormat.Heightmap;
            if (fnamel.EndsWith(".ypdb.xml")) mformat = MetaFormat.Ypdb;
            if (fnamel.EndsWith(".yfd.xml")) mformat = MetaFormat.Yfd;
            if (fnamel.EndsWith(".mrf.xml")) mformat = MetaFormat.Mrf;
            return mformat;
        }
        ...
        private static byte[] Traverse(XmlNode node, MetaBuilder mb, MetaName type = 0, bool isRoot = false)
        {
            ...
            if (infos != null)
            {
                ...
                for (int i = 0; i < infos.Entries.Length; i++)
                {
                    ...
                    switch (entry.DataType)
                    {
                        ...
                        case MetaStructureEntryDataType.DataBlockPointer:
                        {
                            ...
                            string[]
                                split = cnode.InnerText.Trim()
                                    .Split(sa, so); //split = Split(node.InnerText, 2); to read as unsplitted HEX
                            List<byte> bytes = new List<byte>();
                            for (int j = 0; j < split.Length; j++)
                            {
                                byte val; // = Convert.ToByte(split[j], 10);
                                if (byte.TryParse(split[j].Trim(), ns, ic, out val)) bytes.Add(val);
                            }
                        ...
                        case MetaStructureEntryDataType.Structure:
                        {
                            ...
                            if (struc != null) Buffer.BlockCopy(struc, 0, data, entry.DataOffset, struc.Length);
                ...
                if (isRoot) mb.AddItem(type, data);
        ...
        private static void GetParsedArrayOfBytes(XmlNode node, byte[] data, MetaStructureEntryInfo_s entry,
            MetaStructureEntryInfo_s arrEntry)
        {
            int offset = entry.DataOffset;
        ...
        private static void TraverseArray(XmlNode node, MetaBuilder mb, MetaStructureEntryInfo_s arrEntry, int offset,
        ...
        private static Array_Structure TraverseArrayStructure(XmlNode node, MetaBuilder mb, MetaName type)
        {
            ...
            foreach (XmlNode cnode in node.ChildNodes)
            {
                ...
                if (struc != null) strucs.Add(struc);
            }
            ...
            if (cnodes.Count > 0)
            {
                foreach (XmlNode cnode in cnodes)
                {
                    ...
                    if (strs.Length >= 3)
                    {
                        ...
                        if (strs.Length >= 4) w = FloatUtil.Parse(strs[3].Trim());
        ...
        private static XmlNode GetEntryNode(XmlNodeList nodes, MetaStructureEntryInfo_s entry)
        {
            foreach (XmlNode node in nodes)
            {
                ...
                if (GetHash(node.Name) == (uint)entry.EntryNameHash) return node;
            }
        ...
        {
            int intval = 0;
            if (int.TryParse(enumString,
                    out intval))
                return
                    intval; //it's already an int.... maybe enum not found or has no entries... or original value didn't match anything
...

            {
                ...
                for (int j = 0; j < infos.Entries.Length; j++)
                {
                    ...
                    if (entry.EntryNameHash == enumName) return entry.EntryValue;
                }
            }
        ...
        public static MetaHash[] ReadHashItemArray(XmlNode node, string name)
        {
            ...
            if (vnode != null)
            {
                ...
                if (inodes?.Count > 0)
                {
                    ...
                    foreach (XmlNode inode in inodes) vlist.Add(GetHash(inode.InnerText));
                    return vlist.ToArray();
                }
            }
        ...
        public static string[] ReadStringItemArray(XmlNode node, string name)
        {
            ...
            if (vnode != null)
            {
                ...
                if (inodes?.Count > 0)
                {
                    ...
                    foreach (XmlNode inode in inodes) vlist.Add(inode.InnerText);
                    return vlist.ToArray();
                }
            }

...
namespace CodeWalker.GameFiles
{
    public class MetaXml : MetaXmlBase
    {
        public static string GetXml(RpfFileEntry e, byte[] data, out string filename, string outputfolder = "")
        {
            ...
            if (!string.IsNullOrEmpty(outputfolder)) outputfolder = Path.Combine(outputfolder, e.GetShortName());
        ...
        public static string GetXml(YmtFile ymt, out string filename)
        {
            ...
            {
                filename = fn + ".xml";
                return GetXml(ymt.Meta);
            }

            if (ymt.Pso != null)
            {
                filename = fn + ".pso.xml";
                return PsoXml.GetXml(ymt.Pso);
            }

            if (ymt.Rbf != null)
            {
                filename = fn + ".rbf.xml";
                return RbfXml.GetXml(ymt.Rbf);
            }
        ...
        public static string GetXml(YmfFile ymf, out string filename)
        {
            ...
            {
                filename = fn + ".xml";
                return GetXml(ymf.Meta);
            }

            if (ymf.Pso != null)
            {
                filename = fn + ".pso.xml";
                return PsoXml.GetXml(ymf.Pso);
            }

            if (ymf.Rbf != null)
            {
                filename = fn + ".rbf.xml";
                return RbfXml.GetXml(ymf.Rbf);
            }
        ...
        public static string GetXml(Meta meta)
        {
            ...
            if (meta != null)
            {
                ...
                WriteNode(sb, 0, cont, meta.RootBlockIndex, 0, XmlTagMode.Structure, 0, meta.Name);
            }

            return sb.ToString();
        }

        private static void WriteNode(StringBuilder sb, int indent, MetaCont cont, int blockId, int offset,
            XmlTagMode tagMode = XmlTagMode.None, MetaName structName = 0, string metaName = "")
        {
            ...
            if (structName == 0) structName = block.StructureNameHash;
            ...
            for (int i = 0; i < structInfo.Entries.Length; i++)
            {
                ...
                switch (entry.DataType)
                {
                    ...
                    case MetaStructureEntryDataType.Float_XYZ:
                        ...
                        SelfClosingTag(sb, cind,
                            ename + " x=\"" + FloatUtil.ToString(v3.X) + "\" y=\"" + FloatUtil.ToString(v3.Y) +
                            "\" z=\"" + FloatUtil.ToString(v3.Z) + "\"");
                        break;
                    case MetaStructureEntryDataType.Float_XYZW:
                        Vector4 v4 = MetaTypes.ConvertData<Vector4>(data, eoffset);
                        SelfClosingTag(sb, cind,
                            ename + " x=\"" + FloatUtil.ToString(v4.X) + "\" y=\"" + FloatUtil.ToString(v4.Y) +
                            "\" z=\"" + FloatUtil.ToString(v4.Z) + "\" w=\"" + FloatUtil.ToString(v4.W) + "\"");
                        break;
                    ...
                    case MetaStructureEntryDataType.UnsignedShort:
                        ...
                        ValueTag(sb, cind, ename,
                            ushortVal.ToString()); // "0x" + ushortVal.ToString("X").PadLeft(4, '0'));
                        break;
                }
            }
        ...
        private static void WriteArrayNode(StringBuilder sb, int indent, MetaCont cont, byte[] data,
            MetaStructureEntryInfo_s arrEntry, string ename, int eoffset)
        {
            int aCount = 0;
            int aind = indent + 1;
            string arrTag = ename;
            switch (arrEntry.DataType)
            {
                ...
                case MetaStructureEntryDataType.Float_XYZ:
                    ...
                    Vector4[] v4Arr = MetaTypes.ConvertDataArray<Vector4>(cont.Meta, (MetaName)MetaTypeName.VECTOR4,
                        arrV3.Pointer, arrV3.Count1);
        ...
        private static void WriteParsedArrayOfBytesNode(StringBuilder sb, int indent, byte[] data, string ename,
            int eoffset, MetaStructureEntryInfo_s entry, MetaStructureEntryInfo_s arrEntry)
        {
            ...
            switch (arrEntry.DataType)
            {
                ...
                case MetaStructureEntryDataType.SignedInt:
                    for (int n = 0; n < byteArrLen; n++)
                    {
                        ...
                        int b = bidx >= 0 && bidx < data.Length ? BitConverter.ToInt32(data, bidx) : 0;
                        sb.Append(b.ToString());
                        if (n < byteArrLen - 1) sb.Append(" ");
                    }
                ...
                case MetaStructureEntryDataType.UnsignedInt:
                    ...
                    {
                        int bidx = eoffset + n * 4;
                        uint b = bidx >= 0 && bidx < data.Length ? BitConverter.ToUInt32(data, bidx) : 0;
                        sb.Append(b.ToString());
                        if (n < byteArrLen - 1) sb.Append(" ");
                    }
                ...
                case MetaStructureEntryDataType.Float:
                    ...
                    {
                        int bidx = eoffset + n * 4;
                        float b = bidx >= 0 && bidx < data.Length ? BitConverter.ToSingle(data, bidx) : 0;
        ...
        private static string GetEnumString(MetaCont cont, MetaStructureEntryInfo_s entry, int value)
        {
            ...
            if (eInfo == null || eInfo.Entries == null) return value.ToString();
        ...
        private class MetaCont
        {
            ...
            private readonly Dictionary<MetaName, MetaStructureInfo> structInfos =
    ...
    public class PsoXml : MetaXmlBase
    {
        ...
        private static void WriteNode(StringBuilder sb, int indent, PsoCont cont, int blockId, int offset,
            XmlTagMode tagMode = XmlTagMode.None, MetaName structName = 0)
        {
            ...
            if (offset >= block.Length) offset = offset >> 8; //how to tell when to do this??

            int boffset = offset + block.Offset;

            if (structName == 0) structName = block.NameHash;

            string name = HashString(structName);
            byte[] data = cont.Pso.DataSection.Data;

            PsoStructureInfo structInfo = cont.GetStructureInfo(structName);
            if (structInfo == null) structInfo = PsoTypes.GetStructureInfo(structName); //fallback to builtin...
            if (structInfo == null)
            {
                ErrorXml(sb, indent, "Couldn't find structure info " + name + "!");
                return;
            ...
            for (int i = 0; i < structInfo.Entries.Length; i++)
            {
                ...
                if (entry.EntryNameHash == (MetaName)MetaTypeName.ARRAYINFO) continue;
                string ename = HashString(entry.EntryNameHash);
                ...
                switch (entry.Type)
                {
                    ...
                    case PsoDataType.Float2:
                        ...
                        SelfClosingTag(sb, cind,
                            ename + " x=\"" + FloatUtil.ToString(v2.X) + "\" y=\"" + FloatUtil.ToString(v2.Y) + "\"");
                        break;
                    ...
                    case PsoDataType.Float3a: //TODO: check this!
                        ...
                        SelfClosingTag(sb, cind,
                            ename + " x=\"" + FloatUtil.ToString(v3a.X) + "\" y=\"" + FloatUtil.ToString(v3a.Y) +
                            "\" z=\"" + FloatUtil.ToString(v3a.Z) + "\"");
                        break;
                    case PsoDataType.Float4a: //TODO: check this! //...why are there 3 different types of float3?
                        ...
                        SelfClosingTag(sb, cind,
                            ename + " x=\"" + FloatUtil.ToString(v3b.X) + "\" y=\"" + FloatUtil.ToString(v3b.Y) +
                            "\" z=\"" + FloatUtil.ToString(v3b.Z) + "\"");
                        break;
                    ...
                    case PsoDataType.Structure:
                        switch (entry.Unk_5h)
                        {
                            ...
                            case 0: //default structure
                                OpenTag(sb, cind, ename);
                                WriteNode(sb, cind, cont, blockId, offset + entry.DataOffset, XmlTagMode.None,
                                    (MetaName)entry.ReferenceKey);
                                CloseTag(sb, cind, ename);
                                break;
                            ...
                            case 4: //also pointer? what's the difference?
                                ...
                                bool pbok = true;
                                if (pbid <= 0) pbok = false; //no block specified?
                                if (pbid > cont.Pso.DataMapSection
                                        .EntriesCount)
                                ...
                                if (pbok)
                                {
                                    ...
                                    WriteNode(sb, cind, cont, ptrVal.BlockID, (int)ptrVal.ItemOffset, XmlTagMode.None,
                                        (MetaName)entry.ReferenceKey);
                                    CloseTag(sb, cind, ename);
                                }
                                else
        ...
        private static void WriteArrayNode(StringBuilder sb, int indent, PsoCont cont, int blockId, int offset,
            PsoStructureEntryInfo entry, PsoStructureInfo estruct, string ename)
        {
            PsoDataMappingEntry block = cont.Pso.GetBlock(blockId);
            int boffset = offset + block.Offset;
            int eoffset = boffset + entry.DataOffset;
            ...
            if (arrEntInd >= estruct.EntriesCount) arrEntInd = entry.ReferenceKey & 0xFFF;
            ...
            switch (entry.Unk_5h)
            {
                ...
                case 0: //Array_Structure
                    ...
                    if (aBlock != null) abOffset = aOffset + aBlock.Offset;
                    embedded = false;
                    break;
                ...
                case 4: //pointer array? default array?
                    if (arrEntry.Unk_5h == 3) //pointers...
                    {
                        ...
                        if (aBlock2 != null) abOffset = aOffset + aBlock2.Offset;
                        embedded = false;
                    }
            ...
            {
                default:
                    ErrorXml(sb, indent, ename + ": WIP! Unsupported array entry DataType: " + arrEntry.Type);
                    break;
                case PsoDataType.Array:
                ...
                case PsoDataType.Structure:
                            ...
                            if (aCount > 0)
                            {
                                ...
                                for (int n = 0; n < aCount; n++)
                                {
                                    ...
                                    if (ptrVal.Pointer == 0)
                                        ...
                                        WriteNode(sb, aind, cont, ptrVal.BlockID, (int)ptrVal.ItemOffset,
                                            XmlTagMode.ItemAndType);
                                }
                    ...
                    if (aCount > 0)
                    {
                        ...
                        if (aBlock == null)
                        {
                            ErrorXml(sb, indent, ename + ": Array block not found: " + aBlockId);
                        }
                        else if (aBlock.NameHash != (MetaName)MetaTypeName.PsoPOINTER)
                        {
                            ...
                            if (atyp == null)
                                ErrorXml(sb, indent,
                                    ename + ": Array type not found: " + HashString((MetaHash)arrEntry.ReferenceKey));
                            else
                                for (int n = 0; n < aCount; n++)
                                {
                                    WriteNode(sb, aind, cont, aBlockId, aOffset, XmlTagMode.Item,
                                        (MetaName)arrEntry.ReferenceKey);
                                    aOffset += atyp.StructureLength;
                                    if (n < aCount - 1 && aBlock != null && aOffset >= aBlock.Length) break;
                                }
                ...
                case PsoDataType.String:
                    switch (arrEntry.Unk_5h)
                    {
                        ...
                        case 2: //string array  (array of pointers)
                            ...
                            arrStrs.SwapEnd();
                            DataBlockPointer[] strPtrArr =
                                PsoTypes.GetItemArrayRaw<DataBlockPointer>(cont.Pso, arrStrs);
                        ...
                        case 3: //char array array  (array of CharPointer)
                            ...
                            arrChars.SwapEnd();
                            CharPointer[]
                                charPtrArr = PsoTypes.GetItemArrayRaw<CharPointer>(cont.Pso, arrChars); //namehash 200
                ...
                case PsoDataType.Float3:
                    ...
                    arrTag +=
                        " itemType=\"Vector3\""; //this is actually aligned as vector4, the W values are crazy in places
        ...
        private static void WriteMapNode(StringBuilder sb, int indent, PsoCont cont, int eoffset,
            PsoStructureEntryInfo entry, PsoStructureInfo structInfo, string ename)
        {
            int cind = indent + 1;
            byte[] data = cont.Pso.DataSection.Data;
            switch (entry.Unk_5h)
            {
                default:
                    ErrorXml(sb, cind, ename + ": Unexpected Map subtype: " + entry.Unk_5h);
                    break;
                case 1:
                    ...
                    {
                        ErrorXml(sb, cind, ename + ": Couldn't find Map xBlock: " + xBlockId);
                    }
                    else
                    {
                        if (xCount1 != xCount2)
                        ...
                        if (xCount1 > 0)
                        {
                            ...
                            if (xOffset >= xBlock.Length) xOffset = xOffset >> 8; //how to tell when to do this??
                            ...
                            {
                                ErrorXml(sb, aind,
                                    ename + ": Map struct type not found: " + HashString(xBlock.NameHash));
                            }
                            else if
                                (xStruct.IndexInfo ==
                                 null) // || (xStruct.IndexInfo.NameHash != (MetaName)MetaTypeName.ARRAYINFO))
                            {
                                ErrorXml(sb, aind,
                                    ename + ": Map struct was missing IndexInfo! " +
                                    (xStruct == null ? "" : xStruct.ToString()));
                            }
                            ...
                            else
                            {
                                ...
                                for (int n = 0; n < xCount1; n++)
                                {
                                    ...
                                    {
                                        ErrorXml(sb, aind, "Offset out of range! Count is " + xCount1);
                                        break; //out of range...
                                    }
                                    ...
                                    if (iEntry.Type == PsoDataType.String)
                                    {
                                        ...
                                        (iEntry.ReferenceKey !=
                                         0) //(xBlock.NameHash != (MetaName)MetaTypeName.ARRAYINFO)//257,258,259
                                    {
                                        //embedded map values
                                        int vOffset = (int)xOffset + iEntry.DataOffset;
                                        OpenTag(sb, aind,
                                            "Item type=\"" + HashString((MetaName)iEntry.ReferenceKey) + "\" key=\"" +
                                            kStr + "\"");
                                        WriteNode(sb, aind, cont, xBlockId, vOffset, XmlTagMode.None,
                                            (MetaName)iEntry.ReferenceKey);
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            string iStr = "Item type=\"" + HashString(iBlock.NameHash) + "\" key=\"" +
                                                          kStr + "\"";
                                            ...
                                            else
                                            {
                                                ...
                                                OpenTag(sb, aind, iStr);
                                                WriteNode(sb, aind, cont, iPtr.BlockID,
                                                    iOff); //, (MetaName)entry.ReferenceKey);
                                                CloseTag(sb, aind, "Item");
                                            }
                                        }
                                    }
        ...
        public class PsoCont
        {
            ...
            public PsoCont(PsoFile pso)
            {
                ...
                if (pso.SchemaSection == null || pso.SchemaSection.Entries == null ||
                    pso.SchemaSection.EntriesIdx == null) return;
        ...
        private static void WriteNode(StringBuilder sb, int indent, RbfStructure rs)
        {
            ...
            if (rs.Attributes.Count > 0)
            {
                ...
                foreach (IRbfType attr in rs.Attributes)
                    ...
                    else if (attr is RbfBoolean bln) asb.Append($" {attr.Name}=\"{bln.Value.ToString()}\"");
            ...
            foreach (IRbfType child in rs.Children)
            {
                ...
                if (child is RbfStructure) WriteNode(sb, cind, child as RbfStructure);
                ...
                if (child is RbfFloat3)
                {
                    ...
                    SelfClosingTag(sb, cind,
                        v3.Name + " x=\"" + FloatUtil.ToString(v3.X) + "\" y=\"" + FloatUtil.ToString(v3.Y) +
                        "\" z=\"" + FloatUtil.ToString(v3.Z) + "\"");
                }
            }
    ...
    public class MetaXmlBase
    {
        ...
        public static void WriteRawArrayContent<T>(StringBuilder sb, T[] arr, int ind, Func<T, string> formatter = null,
            int arrRowSize = 10) where T : struct
        {
        ...
        public static void WriteRawArray<T>(StringBuilder sb, T[] arr, int ind, string name, string typeName,
            Func<T, string> formatter = null, int arrRowSize = 10) where T : struct
        {
            int aCount = arr?.Length ?? 0;
            //var arrRowSize = 10;
            int aind = ind + 1;
        ...
        public static void WriteItemArray<T>(StringBuilder sb, T[] arr, int ind, string name, string typeName,
            Func<T, string> formatter) where T : struct
        {
            int aCount = arr?.Length ?? 0;
        ...
        public static void WriteCustomItemArray<T>(StringBuilder sb, T[] arr, int ind, string name)
            where T : IMetaXmlItem
        {
            int itemCount = arr?.Length ?? 0;
            if (itemCount > 0)
            {
        ...
        public static string
            FormatVector4SwapXYZOnly(
                Vector4 v) //for use with WriteItemArray, swaps endianness, and outputs only XYZ components
        {

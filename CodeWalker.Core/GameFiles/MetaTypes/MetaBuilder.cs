...
namespace CodeWalker.GameFiles
{
    public class MetaBuilder
    {
        ...
        {
            MetaBuilderPointer ptr = AddItem(type, item);
        ...
        public Array_Structure AddItemArrayPtr(MetaName type, byte[][] data) //helper method for AddItemArray<T>
        {
            ...
            MetaBuilderPointer ptr = AddItemArray(type, newdata, data.Length);
            return new Array_Structure(ptr);
        }
        ...
        public Array_Structure AddWrapperArray(MetaWrapper[] items)
        {
            ...
            for (int i = 0; i < items.Length; i++)
            {
                MetaWrapper item = items[i];
                MetaPOINTER meptr = item.Save(this);
                if (i == 0)
                {
        ...
        public byte[] GetData()
        {
            ...
            {
                MetaBuilderBlock block = Blocks[i];
                for (int j = 0; j < block.Items.Count; j++)
                {
                    byte[] bdata = block.Items[j];

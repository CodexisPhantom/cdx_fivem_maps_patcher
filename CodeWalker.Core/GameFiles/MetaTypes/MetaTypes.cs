...
namespace CodeWalker.GameFiles
{
    ...
    public static class MetaTypes
    {
        ...
        public static string GetTypesString()
        {
            ...
            foreach (KeyValuePair<uint, MetaEnumInfo> kvp in EnumDict)
            {
                MetaEnumInfo mei = kvp.Value;
                string name = GetSafeName(mei.EnumNameHash, mei.EnumKey);
                sbe.AppendLine("public enum " + name + " //Key:" + mei.EnumKey);
                sbe.AppendLine("{");
                foreach (MetaEnumEntryInfo_s entry in mei.Entries)
                {
            ...
            foreach (KeyValuePair<uint, MetaStructureInfo> kvp in StructDict)
            {
                MetaStructureInfo msi = kvp.Value;
                string name = GetSafeName(msi.StructureNameHash, msi.StructureKey);
                sbs.AppendLine("public struct " + name + " //" + msi.StructureSize + " bytes, Key:" + msi.StructureKey);
                sbs.AppendLine("{");
                for (int i = 0; i < msi.Entries.Length; i++)
                {
                    MetaStructureEntryInfo_s entry = msi.Entries[i];

                    if (entry.DataOffset == 0 &&
                        entry.EntryNameHash == (MetaName)MetaTypeName.ARRAYINFO) //referred to by array
                    {
                    }
                    else
                    {
                        ...
                        if (entry.DataType == MetaStructureEntryDataType.Array)
                        {
                            ...
                            {
                                MetaStructureEntryInfo_s structentry = msi.Entries[entry.ReferenceTypeIndex];
                                string typename = "Array_" +
                                                  MetaStructureEntryDataTypes.GetCSharpTypeName(structentry.DataType);
                                sbs.AppendFormat(fmt, typename, sename, entry.DataOffset,
                                    entry + "  {" + structentry + "}");
                                sbs.AppendLine();
                            }
                        }
                        ...
                        {
                            string typename = GetSafeName(entry.ReferenceKey, (uint)entry.ReferenceTypeIndex);
                            sbs.AppendFormat(fmt, typename, sename, entry.DataOffset, entry.ToString());
                            sbs.AppendLine();
                        }
                        else
                        {
                            string typename = MetaStructureEntryDataTypes.GetCSharpTypeName(entry.DataType);
        ...
        public static string GetTypesInitString(Meta meta)
        {
            ...
            foreach (MetaStructureInfo si in meta.StructureInfos) AddStructureInfoString(si, sb);

            sb.AppendLine();

            foreach (MetaEnumInfo ei in meta.EnumInfos) AddEnumInfoString(ei, sb);
        ...
        public static string GetTypesInitString()
        {
            ...
            foreach (MetaStructureInfo si in StructDict.Values) AddStructureInfoString(si, sb);

            sb.AppendLine();

            foreach (MetaEnumInfo ei in EnumDict.Values) AddEnumInfoString(ei, sb);
        ...
        {
            string ns = GetMetaNameString(si.StructureNameHash);
            sb.AppendFormat("case " + ns + ":");
            sb.AppendLine();
            sb.AppendFormat("return new MetaStructureInfo({0}, {1}, {2}, {3},", ns, si.StructureKey, si.Unknown_8h,
                si.StructureSize);
            sb.AppendLine();
            for (int i = 0; i < si.Entries.Length; i++)
            {
                MetaStructureEntryInfo_s e = si.Entries[i];
                string refkey = "0";
                if (e.ReferenceKey != 0) refkey = GetMetaNameString(e.ReferenceKey);
                sb.AppendFormat(
                    " new MetaStructureEntryInfo_s({0}, {1}, MetaStructureEntryDataType.{2}, {3}, {4}, {5})",
                    GetMetaNameString(e.EntryNameHash), e.DataOffset, e.DataType, e.Unknown_9h, e.ReferenceTypeIndex,
                    refkey);
        ...
        {
            string ns = GetMetaNameString(ei.EnumNameHash);
            sb.AppendFormat("case " + ns + ":");
            sb.AppendLine();
            ...
            {
                MetaEnumEntryInfo_s e = ei.Entries[i];
        ...
        {
            if (Enum.IsDefined(typeof(MetaName), n)) return "MetaName." + n;
            ...
            switch (name)
            {
                case MetaName.CScenarioPointContainer:
                    return new MetaStructureInfo(MetaName.CScenarioPointContainer, 2489654897, 768, 48,
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CExtensionDefSpawnPoint),
                        new MetaStructureEntryInfo_s(MetaName.LoadSavePoints, 0, MetaStructureEntryDataType.Array, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CScenarioPoint),
                ...
                case MetaName.CScenarioChainingGraph:
                    return new MetaStructureInfo(MetaName.CScenarioChainingGraph, 88255871, 768, 88,
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CScenarioChainingNode),
                        new MetaStructureEntryInfo_s(MetaName.Nodes, 0, MetaStructureEntryDataType.Array, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CScenarioChainingEdge),
                        new MetaStructureEntryInfo_s(MetaName.Edges, 16, MetaStructureEntryDataType.Array, 0, 2, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CScenarioChain),
                ...
                case MetaName.rage__spdGrid2D:
                    return new MetaStructureInfo(MetaName.rage__spdGrid2D, 894636096, 768, 64,
                        new MetaStructureEntryInfo_s(MetaName.MinCellX, 12, MetaStructureEntryDataType.SignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.MaxCellX, 16, MetaStructureEntryDataType.SignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.MinCellY, 20, MetaStructureEntryDataType.SignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.MaxCellY, 24, MetaStructureEntryDataType.SignedInt, 0, 0,
                            0),
                ...
                case MetaName.CScenarioPointLookUps:
                    return new MetaStructureInfo(MetaName.CScenarioPointLookUps, 2669361587, 768, 96,
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.TypeNames, 0, MetaStructureEntryDataType.Array, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.PedModelSetNames, 16, MetaStructureEntryDataType.Array, 0,
                            2, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.VehicleModelSetNames, 32,
                            MetaStructureEntryDataType.Array, 0, 4, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.GroupNames, 48, MetaStructureEntryDataType.Array, 0, 6,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.InteriorNames, 64, MetaStructureEntryDataType.Array, 0, 8,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.RequiredIMapNames, 80, MetaStructureEntryDataType.Array,
                            0, 10, 0)
                    );
                case MetaName.CScenarioPointRegion:
                    return new MetaStructureInfo(MetaName.CScenarioPointRegion, 3501351821, 768, 376,
                        new MetaStructureEntryInfo_s(MetaName.VersionNumber, 0, MetaStructureEntryDataType.SignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.Points, 8, MetaStructureEntryDataType.Structure, 0, 0,
                            MetaName.CScenarioPointContainer),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CScenarioEntityOverride),
                        new MetaStructureEntryInfo_s(MetaName.EntityOverrides, 72, MetaStructureEntryDataType.Array, 0,
                            2, 0),
                        new MetaStructureEntryInfo_s(MetaName.ChainingGraph, 96, MetaStructureEntryDataType.Structure,
                            0, 0, MetaName.CScenarioChainingGraph),
                        new MetaStructureEntryInfo_s(MetaName.AccelGrid, 184, MetaStructureEntryDataType.Structure, 0,
                            0, MetaName.rage__spdGrid2D),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedShort, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)3844724227, 248, MetaStructureEntryDataType.Array, 0, 6,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CScenarioPointCluster),
                        new MetaStructureEntryInfo_s(MetaName.Clusters, 264, MetaStructureEntryDataType.Array, 0, 8, 0),
                        new MetaStructureEntryInfo_s(MetaName.LookUps, 280, MetaStructureEntryDataType.Structure, 0, 0,
                ...
                case MetaName.CScenarioPoint:
                    return new MetaStructureInfo(MetaName.CScenarioPoint, 402442150, 1024, 64,
                        new MetaStructureEntryInfo_s(MetaName.iType, 21, MetaStructureEntryDataType.UnsignedByte, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.ModelSetId, 22, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.iInterior, 23, MetaStructureEntryDataType.UnsignedByte, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.iRequiredIMapId, 24,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.iProbability, 25, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.uAvailableInMpSp, 26,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.iTimeStartOverride, 27,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.iTimeEndOverride, 28,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.iRadius, 29, MetaStructureEntryDataType.UnsignedByte, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.iTimeTillPedLeaves, 30,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.iScenarioGroup, 32,
                            MetaStructureEntryDataType.UnsignedShort, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.Flags, 36, MetaStructureEntryDataType.IntFlags2, 0, 32,
                            MetaName.CScenarioPointFlags__Flags),
                        new MetaStructureEntryInfo_s(MetaName.vPositionAndDirection, 48,
                            MetaStructureEntryDataType.Float_XYZW, 0, 0, 0)
                    );
                case MetaName.CScenarioEntityOverride:
                    return new MetaStructureInfo(MetaName.CScenarioEntityOverride, 1271200492, 1024, 80,
                        new MetaStructureEntryInfo_s(MetaName.EntityPosition, 0, MetaStructureEntryDataType.Float_XYZ,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.EntityType, 16, MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CExtensionDefSpawnPoint),
                        new MetaStructureEntryInfo_s(MetaName.ScenarioPoints, 24, MetaStructureEntryDataType.Array, 0,
                            2, 0),
                        new MetaStructureEntryInfo_s(MetaName.EntityMayNotAlwaysExist, 64,
                            MetaStructureEntryDataType.Boolean, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.SpecificallyPreventArtPoints, 65,
                            MetaStructureEntryDataType.Boolean, 0, 0, 0)
                    );
                case MetaName.CExtensionDefSpawnPoint:
                    return new MetaStructureInfo(MetaName.CExtensionDefSpawnPoint, 3077340721, 1024, 96,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetPosition, 16, MetaStructureEntryDataType.Float_XYZ,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.offsetRotation, 32, MetaStructureEntryDataType.Float_XYZW,
                            0, 0, 0),
                        ...
                        new MetaStructureEntryInfo_s(MetaName.requiredImap, 64, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.availableInMpSp, 68, MetaStructureEntryDataType.IntEnum,
                            0, 0, MetaName.CSpawnPoint__AvailabilityMpSp),
                        new MetaStructureEntryInfo_s(MetaName.probability, 72, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.timeTillPedLeaves, 76, MetaStructureEntryDataType.Float,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.radius, 80, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.start, 84, MetaStructureEntryDataType.UnsignedByte, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.end, 85, MetaStructureEntryDataType.UnsignedByte, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 88, MetaStructureEntryDataType.IntFlags2, 0, 32,
                            MetaName.CScenarioPointFlags__Flags),
                        new MetaStructureEntryInfo_s(MetaName.highPri, 92, MetaStructureEntryDataType.Boolean, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.extendedRange, 93, MetaStructureEntryDataType.Boolean, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.shortRange, 94, MetaStructureEntryDataType.Boolean, 0, 0,
                            0)
                    );
                case MetaName.CScenarioChainingNode:
                    return new MetaStructureInfo(MetaName.CScenarioChainingNode, 1811784424, 1024, 32,
                        new MetaStructureEntryInfo_s(MetaName.Position, 0, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)2602393771, 16, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.ScenarioType, 20, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.HasIncomingEdges, 24, MetaStructureEntryDataType.Boolean,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.HasOutgoingEdges, 25, MetaStructureEntryDataType.Boolean,
                            0, 0, 0)
                    );
                case MetaName.CScenarioChainingEdge:
                    return new MetaStructureInfo(MetaName.CScenarioChainingEdge, 2004985940, 256, 8,
                        new MetaStructureEntryInfo_s(MetaName.NodeIndexFrom, 0,
                            MetaStructureEntryDataType.UnsignedShort, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.NodeIndexTo, 2, MetaStructureEntryDataType.UnsignedShort,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.Action, 4, MetaStructureEntryDataType.ByteEnum, 0, 0,
                            MetaName.CScenarioChainingEdge__eAction),
                        new MetaStructureEntryInfo_s(MetaName.NavMode, 5, MetaStructureEntryDataType.ByteEnum, 0, 0,
                            MetaName.CScenarioChainingEdge__eNavMode),
                        new MetaStructureEntryInfo_s(MetaName.NavSpeed, 6, MetaStructureEntryDataType.ByteEnum, 0, 0,
                            MetaName.CScenarioChainingEdge__eNavSpeed)
                    );
                case MetaName.CScenarioChain:
                    return new MetaStructureInfo(MetaName.CScenarioChain, 2751910366, 768, 40,
                        new MetaStructureEntryInfo_s((MetaName)1156691834, 0, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedShort, 0, 0, 0),
                ...
                case MetaName.rage__spdSphere:
                    return new MetaStructureInfo(MetaName.rage__spdSphere, 1189037266, 1024, 16,
                        new MetaStructureEntryInfo_s(MetaName.centerAndRadius, 0, MetaStructureEntryDataType.Float_XYZW,
                            0, 0, 0)
                    );
                case MetaName.CScenarioPointCluster:
                    return new MetaStructureInfo(MetaName.CScenarioPointCluster, 3622480419, 1024, 80,
                        new MetaStructureEntryInfo_s(MetaName.Points, 0, MetaStructureEntryDataType.Structure, 0, 0,
                            MetaName.CScenarioPointContainer),
                        new MetaStructureEntryInfo_s(MetaName.ClusterSphere, 48, MetaStructureEntryDataType.Structure,
                            0, 0, MetaName.rage__spdSphere),
                        new MetaStructureEntryInfo_s((MetaName)1095875445, 64, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)3129415068, 68, MetaStructureEntryDataType.Boolean, 0, 0,
                            0)
                    );
                case MetaName.CStreamingRequestRecord:
                    return new MetaStructureInfo(MetaName.CStreamingRequestRecord, 3825587854, 768, 40,
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CStreamingRequestFrame),
                        new MetaStructureEntryInfo_s(MetaName.Frames, 0, MetaStructureEntryDataType.Array, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CStreamingRequestCommonSet),
                        new MetaStructureEntryInfo_s(MetaName.CommonSets, 16, MetaStructureEntryDataType.Array, 0, 2,
                            0),
                ...
                case MetaName.CStreamingRequestFrame:
                    return new MetaStructureInfo(MetaName.CStreamingRequestFrame, 1112444512, 1024, 112,
                        ...
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.RemoveList, 16, MetaStructureEntryDataType.Array, 0, 2,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.PromoteToHDList, 32, MetaStructureEntryDataType.Array, 0,
                            4, 0),
                        new MetaStructureEntryInfo_s(MetaName.CamPos, 48, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.CamDir, 64, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.CommonAddSets, 80, MetaStructureEntryDataType.Array, 0, 8,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.Flags, 96, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0)
                    );
                //case MetaName.CStreamingRequestFrame:
                ...
                case MetaName.CMapTypes:
                    return new MetaStructureInfo(MetaName.CMapTypes, 2608875220, 768, 80,
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.StructurePointer, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.extensions, 8, MetaStructureEntryDataType.Array, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.StructurePointer, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.archetypes, 24, MetaStructureEntryDataType.Array, 0, 2,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.name, 40, MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.dependencies, 48, MetaStructureEntryDataType.Array, 0, 5,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CCompositeEntityType),
                        new MetaStructureEntryInfo_s(MetaName.compositeEntityTypes, 64,
                            MetaStructureEntryDataType.Array, 0, 7, 0)
                    );
                case MetaName.CBaseArchetypeDef:
                    return new MetaStructureInfo(MetaName.CBaseArchetypeDef, 2411387556, 1024, 144,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.flags, 12, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.specialAttribute, 16,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0),
                        ...
                        new MetaStructureEntryInfo_s(MetaName.bsCentre, 64, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.bsRadius, 80, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.hdTextureDist, 84, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.name, 88, MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.textureDictionary, 92, MetaStructureEntryDataType.Hash, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.clipDictionary, 96, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.drawableDictionary, 100, MetaStructureEntryDataType.Hash,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.physicsDictionary, 104, MetaStructureEntryDataType.Hash,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.assetType, 108, MetaStructureEntryDataType.IntEnum, 0, 0,
                            MetaName.rage__fwArchetypeDef__eAssetType),
                        new MetaStructureEntryInfo_s(MetaName.assetName, 112, MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.StructurePointer, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.extensions, 120, MetaStructureEntryDataType.Array, 0, 15,
                            0)
                    );
                //case MetaName.CBaseArchetypeDef:
                ...
                case MetaName.CCreatureMetaData:
                    return new MetaStructureInfo(MetaName.CCreatureMetaData, 2181653572, 768, 56,
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CShaderVariableComponent),
                        new MetaStructureEntryInfo_s(MetaName.shaderVariableComponents, 8,
                            MetaStructureEntryDataType.Array, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CPedPropExpressionData),
                        new MetaStructureEntryInfo_s(MetaName.pedPropExpressions, 24, MetaStructureEntryDataType.Array,
                            0, 2, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CPedCompExpressionData),
                        new MetaStructureEntryInfo_s(MetaName.pedCompExpressions, 40, MetaStructureEntryDataType.Array,
                            0, 4, 0)
                    );
                case MetaName.CShaderVariableComponent:
                    return new MetaStructureInfo(MetaName.CShaderVariableComponent, 3085831725, 768, 72,
                        new MetaStructureEntryInfo_s(MetaName.pedcompID, 8, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.maskID, 12, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.shaderVariableHashString, 16,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                ...
                case MetaName.CPedPropExpressionData:
                    return new MetaStructureInfo(MetaName.CPedPropExpressionData, 1355135810, 768, 88,
                        new MetaStructureEntryInfo_s(MetaName.pedPropID, 8, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.pedPropVarIndex, 12, MetaStructureEntryDataType.SignedInt,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.pedPropExpressionIndex, 16,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.tracks, 24, MetaStructureEntryDataType.Array, 0, 3, 0),
                ...
                case MetaName.CPedCompExpressionData:
                    return new MetaStructureInfo(MetaName.CPedCompExpressionData, 3458164745, 768, 88,
                        new MetaStructureEntryInfo_s(MetaName.pedCompID, 8, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.pedCompVarIndex, 12, MetaStructureEntryDataType.SignedInt,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.pedCompExpressionIndex, 16,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.tracks, 24, MetaStructureEntryDataType.Array, 0, 3, 0),
                ...
                case MetaName.rage__fwInstancedMapData:
                    return new MetaStructureInfo(MetaName.rage__fwInstancedMapData, 1836780118, 768, 48,
                        ...
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.rage__fwPropInstanceListDef),
                        new MetaStructureEntryInfo_s(MetaName.PropInstanceList, 16, MetaStructureEntryDataType.Array, 0,
                            1, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.rage__fwGrassInstanceListDef),
                        new MetaStructureEntryInfo_s(MetaName.GrassInstanceList, 32, MetaStructureEntryDataType.Array,
                            0, 3, 0)
                    );
                case MetaName.CLODLight:
                    return new MetaStructureInfo(MetaName.CLODLight, 2325189228, 768, 136,
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.FloatXYZ),
                        new MetaStructureEntryInfo_s(MetaName.direction, 8, MetaStructureEntryDataType.Array, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.falloff, 24, MetaStructureEntryDataType.Array, 0, 2, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.falloffExponent, 40, MetaStructureEntryDataType.Array, 0,
                            4, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.timeAndStateFlags, 56, MetaStructureEntryDataType.Array,
                            0, 6, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0),
                        ...
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.coneInnerAngle, 88, MetaStructureEntryDataType.Array, 0,
                            10, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.coneOuterAngleOrCapExt, 104,
                            MetaStructureEntryDataType.Array, 0, 12, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.coronaIntensity, 120, MetaStructureEntryDataType.Array, 0,
                            14, 0)
                    );
                case MetaName.CDistantLODLight:
                    return new MetaStructureInfo(MetaName.CDistantLODLight, 2820908419, 768, 48,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.numStreetLights, 40,
                            MetaStructureEntryDataType.UnsignedShort, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.category, 42, MetaStructureEntryDataType.UnsignedShort, 0,
                            0, 0)
                    );
                case MetaName.CBlockDesc:
                    return new MetaStructureInfo(MetaName.CBlockDesc, 2015795449, 768, 72,
                        new MetaStructureEntryInfo_s(MetaName.version, 0, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 4, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.name, 8, MetaStructureEntryDataType.CharPointer, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.exportedBy, 24, MetaStructureEntryDataType.CharPointer, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.owner, 40, MetaStructureEntryDataType.CharPointer, 0, 0,
                            0),
                ...
                case MetaName.CMapData:
                    return new MetaStructureInfo(MetaName.CMapData, 3448101671, 1024, 512,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.flags, 16, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.contentFlags, 20, MetaStructureEntryDataType.UnsignedInt,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.streamingExtentsMin, 32,
                            MetaStructureEntryDataType.Float_XYZ, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.streamingExtentsMax, 48,
                            MetaStructureEntryDataType.Float_XYZ, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.entitiesExtentsMin, 64,
                            MetaStructureEntryDataType.Float_XYZ, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.entitiesExtentsMax, 80,
                            MetaStructureEntryDataType.Float_XYZ, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.StructurePointer, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.entities, 96, MetaStructureEntryDataType.Array, 0, 8, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.rage__fwContainerLodDef),
                        new MetaStructureEntryInfo_s(MetaName.containerLods, 112, MetaStructureEntryDataType.Array, 0,
                            10, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.BoxOccluder),
                        new MetaStructureEntryInfo_s(MetaName.boxOccluders, 128, MetaStructureEntryDataType.Array, 4,
                            12, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.OccludeModel),
                        new MetaStructureEntryInfo_s(MetaName.occludeModels, 144, MetaStructureEntryDataType.Array, 4,
                            14, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.physicsDictionaries, 160,
                            MetaStructureEntryDataType.Array, 0, 16, 0),
                        new MetaStructureEntryInfo_s(MetaName.instancedData, 176, MetaStructureEntryDataType.Structure,
                            0, 0, MetaName.rage__fwInstancedMapData),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CTimeCycleModifier),
                        new MetaStructureEntryInfo_s(MetaName.timeCycleModifiers, 224, MetaStructureEntryDataType.Array,
                            0, 19, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CCarGen),
                        new MetaStructureEntryInfo_s(MetaName.carGenerators, 240, MetaStructureEntryDataType.Array, 0,
                            21, 0),
                        new MetaStructureEntryInfo_s(MetaName.LODLightsSOA, 256, MetaStructureEntryDataType.Structure,
                            0, 0, MetaName.CLODLight),
                        new MetaStructureEntryInfo_s(MetaName.DistantLODLightsSOA, 392,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CDistantLODLight),
                        new MetaStructureEntryInfo_s(MetaName.block, 440, MetaStructureEntryDataType.Structure, 0, 0,
                            MetaName.CBlockDesc)
                    );
                case MetaName.CEntityDef:
                    return new MetaStructureInfo(MetaName.CEntityDef, 1825799514, 1024, 128,
                        new MetaStructureEntryInfo_s(MetaName.archetypeName, 8, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 12, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.guid, 16, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.position, 32, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.rotation, 48, MetaStructureEntryDataType.Float_XYZW, 0, 0,
                            0),
                        ...
                        new MetaStructureEntryInfo_s(MetaName.parentIndex, 72, MetaStructureEntryDataType.SignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.lodDist, 76, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.childLodDist, 80, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.lodLevel, 84, MetaStructureEntryDataType.IntEnum, 0, 0,
                            MetaName.rage__eLodType),
                        new MetaStructureEntryInfo_s(MetaName.numChildren, 88, MetaStructureEntryDataType.UnsignedInt,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.priorityLevel, 92, MetaStructureEntryDataType.IntEnum, 0,
                            0, MetaName.rage__ePriorityLevel),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.StructurePointer, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.extensions, 96, MetaStructureEntryDataType.Array, 0, 13,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.ambientOcclusionMultiplier, 112,
                            MetaStructureEntryDataType.SignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.artificialAmbientOcclusion, 116,
                            MetaStructureEntryDataType.SignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.tintValue, 120, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0)
                    );
                case MetaName.CTimeCycleModifier:
                    return new MetaStructureInfo(MetaName.CTimeCycleModifier, 2683420777, 1024, 64,
                        new MetaStructureEntryInfo_s(MetaName.name, 8, MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.minExtents, 16, MetaStructureEntryDataType.Float_XYZ, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.maxExtents, 32, MetaStructureEntryDataType.Float_XYZ, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.percentage, 48, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.range, 52, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.startHour, 56, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.endHour, 60, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0)
                    );
                case MetaName.CTimeArchetypeDef:
                    return new MetaStructureInfo(MetaName.CTimeArchetypeDef, 2520619910, 1024, 160,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.extensions, 120, MetaStructureEntryDataType.Array, 0, 15,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.timeFlags, 144, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0)
                    );
                case MetaName.CExtensionDefLightEffect:
                    return new MetaStructureInfo(MetaName.CExtensionDefLightEffect, 2436199897, 1024, 48,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetPosition, 16, MetaStructureEntryDataType.Float_XYZ,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CLightAttrDef),
                ...
                case MetaName.CLightAttrDef:
                    return new MetaStructureInfo(MetaName.CLightAttrDef, 2363260268, 768, 160,
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.posn, 8, MetaStructureEntryDataType.ArrayOfBytes, 0, 0,
                            (MetaName)3),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.colour, 20, MetaStructureEntryDataType.ArrayOfBytes, 0, 2,
                            (MetaName)3),
                        new MetaStructureEntryInfo_s(MetaName.flashiness, 23, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.intensity, 24, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 28, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.boneTag, 32, MetaStructureEntryDataType.SignedShort, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.lightType, 34, MetaStructureEntryDataType.UnsignedByte, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.groupId, 35, MetaStructureEntryDataType.UnsignedByte, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.timeFlags, 36, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.falloff, 40, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.falloffExponent, 44, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.cullingPlane, 48, MetaStructureEntryDataType.ArrayOfBytes,
                            0, 13, (MetaName)4),
                        new MetaStructureEntryInfo_s(MetaName.shadowBlur, 64, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.padding1, 65, MetaStructureEntryDataType.UnsignedByte, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.padding2, 66, MetaStructureEntryDataType.SignedShort, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.padding3, 68, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.volIntensity, 72, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.volSizeScale, 76, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.volOuterColour, 80,
                            MetaStructureEntryDataType.ArrayOfBytes, 0, 21, (MetaName)3),
                        new MetaStructureEntryInfo_s(MetaName.lightHash, 83, MetaStructureEntryDataType.UnsignedByte, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.volOuterIntensity, 84, MetaStructureEntryDataType.Float,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.coronaSize, 88, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.volOuterExponent, 92, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.lightFadeDistance, 96,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.shadowFadeDistance, 97,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.specularFadeDistance, 98,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.volumetricFadeDistance, 99,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.shadowNearClip, 100, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.coronaIntensity, 104, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.coronaZBias, 108, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.direction, 112, MetaStructureEntryDataType.ArrayOfBytes,
                            0, 34, (MetaName)3),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.tangent, 124, MetaStructureEntryDataType.ArrayOfBytes, 0,
                            36, (MetaName)3),
                        new MetaStructureEntryInfo_s(MetaName.coneInnerAngle, 136, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.coneOuterAngle, 140, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.extents, 144, MetaStructureEntryDataType.ArrayOfBytes, 0,
                            40, (MetaName)3),
                        new MetaStructureEntryInfo_s(MetaName.projectedTextureKey, 156,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0)
                    );
                case MetaName.CMloInstanceDef:
                    return new MetaStructureInfo(MetaName.CMloInstanceDef, 2151576752, 1024, 160,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.tintValue, 120, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.groupId, 128, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.floorId, 132, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.defaultEntitySets, 136, MetaStructureEntryDataType.Array,
                            0, 20, 0),
                        new MetaStructureEntryInfo_s(MetaName.numExitPortals, 152,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.MLOInstflags, 156, MetaStructureEntryDataType.UnsignedInt,
                            0, 0, 0)
                    );
                case MetaName.BoxOccluder:
                    return new MetaStructureInfo(MetaName.BoxOccluder, 1831736438, 256, 16,
                        new MetaStructureEntryInfo_s(MetaName.iCenterX, 0, MetaStructureEntryDataType.SignedShort, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.iCenterY, 2, MetaStructureEntryDataType.SignedShort, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.iCenterZ, 4, MetaStructureEntryDataType.SignedShort, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.iCosZ, 6, MetaStructureEntryDataType.SignedShort, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.iLength, 8, MetaStructureEntryDataType.SignedShort, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.iWidth, 10, MetaStructureEntryDataType.SignedShort, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.iHeight, 12, MetaStructureEntryDataType.SignedShort, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.iSinZ, 14, MetaStructureEntryDataType.SignedShort, 0, 0,
                            0)
                    );
                case MetaName.OccludeModel:
                    return new MetaStructureInfo(MetaName.OccludeModel, 1172796107, 1024, 64,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.dataSize, 32, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.verts, 40, MetaStructureEntryDataType.DataBlockPointer, 4,
                            3, (MetaName)2),
                        new MetaStructureEntryInfo_s(MetaName.numVertsInBytes, 48,
                            MetaStructureEntryDataType.UnsignedShort, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.numTris, 50, MetaStructureEntryDataType.UnsignedShort, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 52, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0)
                    );
                case MetaName.CMloArchetypeDef:
                    return new MetaStructureInfo(MetaName.CMloArchetypeDef, 937664754, 1024, 240,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.extensions, 120, MetaStructureEntryDataType.Array, 0, 15,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.mloFlags, 144, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.StructurePointer, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.entities, 152, MetaStructureEntryDataType.Array, 0, 18,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CMloRoomDef),
                        new MetaStructureEntryInfo_s(MetaName.rooms, 168, MetaStructureEntryDataType.Array, 0, 20, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CMloPortalDef),
                        new MetaStructureEntryInfo_s(MetaName.portals, 184, MetaStructureEntryDataType.Array, 0, 22, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CMloEntitySet),
                        new MetaStructureEntryInfo_s(MetaName.entitySets, 200, MetaStructureEntryDataType.Array, 0, 24,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CMloTimeCycleModifier),
                        new MetaStructureEntryInfo_s(MetaName.timeCycleModifiers, 216, MetaStructureEntryDataType.Array,
                            0, 26, 0)
                    );
                case MetaName.CMloRoomDef:
                    return new MetaStructureInfo(MetaName.CMloRoomDef, 3885428245, 1024, 112,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.timecycleName, 68, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.secondaryTimecycleName, 72,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 76, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.portalCount, 80, MetaStructureEntryDataType.UnsignedInt,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.floorId, 84, MetaStructureEntryDataType.SignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.exteriorVisibiltyDepth, 88,
                            MetaStructureEntryDataType.SignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.attachedObjects, 96, MetaStructureEntryDataType.Array, 0,
                            10, 0)
                    );
                case MetaName.CMloPortalDef:
                    return new MetaStructureInfo(MetaName.CMloPortalDef, 1110221513, 768, 64,
                        new MetaStructureEntryInfo_s(MetaName.roomFrom, 8, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.roomTo, 12, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 16, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.mirrorPriority, 20,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.opacity, 24, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.audioOcclusion, 28,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Float_XYZ, 0, 0, 0),
                        ...
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.attachedObjects, 48, MetaStructureEntryDataType.Array, 0,
                            8, 0)
                    );
                case MetaName.CMloTimeCycleModifier:
                    return new MetaStructureInfo(MetaName.CMloTimeCycleModifier, 838874674, 1024, 48,
                        new MetaStructureEntryInfo_s(MetaName.name, 8, MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.sphere, 16, MetaStructureEntryDataType.Float_XYZW, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.percentage, 32, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.range, 36, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.startHour, 40, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.endHour, 44, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0)
                    );
                case MetaName.CExtensionDefParticleEffect:
                    return new MetaStructureInfo(MetaName.CExtensionDefParticleEffect, 466596385, 1024, 96,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetRotation, 32, MetaStructureEntryDataType.Float_XYZW,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.fxName, 48, MetaStructureEntryDataType.CharPointer, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.fxType, 64, MetaStructureEntryDataType.SignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.boneTag, 68, MetaStructureEntryDataType.SignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.scale, 72, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.probability, 76, MetaStructureEntryDataType.SignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 80, MetaStructureEntryDataType.SignedInt, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.color, 84, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0)
                    );
                case MetaName.CCompositeEntityType:
                    return new MetaStructureInfo(MetaName.CCompositeEntityType, 659539004, 1024, 304,
                        new MetaStructureEntryInfo_s(MetaName.Name, 0, MetaStructureEntryDataType.ArrayOfChars, 0, 0,
                            (MetaName)64),
                        new MetaStructureEntryInfo_s(MetaName.lodDist, 64, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 68, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.specialAttribute, 72,
                            MetaStructureEntryDataType.UnsignedInt, 0, 0, 0),
                        ...
                        new MetaStructureEntryInfo_s(MetaName.bsCentre, 112, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.bsRadius, 128, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.StartModel, 136, MetaStructureEntryDataType.ArrayOfChars,
                            0, 0, (MetaName)64),
                        new MetaStructureEntryInfo_s(MetaName.EndModel, 200, MetaStructureEntryDataType.ArrayOfChars, 0,
                            0, (MetaName)64),
                        new MetaStructureEntryInfo_s(MetaName.StartImapFile, 264, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.EndImapFile, 268, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.PtFxAssetName, 272, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CCompEntityAnims),
                        new MetaStructureEntryInfo_s(MetaName.Animations, 280, MetaStructureEntryDataType.Array, 0, 13,
                            0)
                    );
                case MetaName.CCompEntityAnims:
                    return new MetaStructureInfo(MetaName.CCompEntityAnims, 4110496011, 768, 216,
                        new MetaStructureEntryInfo_s(MetaName.AnimDict, 0, MetaStructureEntryDataType.ArrayOfChars, 0,
                            0, (MetaName)64),
                        new MetaStructureEntryInfo_s(MetaName.AnimName, 64, MetaStructureEntryDataType.ArrayOfChars, 0,
                            0, (MetaName)64),
                        new MetaStructureEntryInfo_s(MetaName.AnimatedModel, 128,
                            MetaStructureEntryDataType.ArrayOfChars, 0, 0, (MetaName)64),
                        new MetaStructureEntryInfo_s(MetaName.punchInPhase, 192, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.punchOutPhase, 196, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CCompEntityEffectsData),
                        new MetaStructureEntryInfo_s(MetaName.effectsData, 200, MetaStructureEntryDataType.Array, 0, 5,
                            0)
                    );
                case MetaName.CCompEntityEffectsData:
                    return new MetaStructureInfo(MetaName.CCompEntityEffectsData, 1724963966, 1024, 160,
                        new MetaStructureEntryInfo_s(MetaName.fxType, 0, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.fxOffsetPos, 16, MetaStructureEntryDataType.Float_XYZ, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.fxOffsetRot, 32, MetaStructureEntryDataType.Float_XYZW, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.boneTag, 48, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.startPhase, 52, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.endPhase, 56, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.ptFxIsTriggered, 60, MetaStructureEntryDataType.Boolean,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.ptFxTag, 61, MetaStructureEntryDataType.ArrayOfChars, 0,
                            0, (MetaName)64),
                        new MetaStructureEntryInfo_s(MetaName.ptFxScale, 128, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.ptFxProbability, 132, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.ptFxHasTint, 136, MetaStructureEntryDataType.Boolean, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.ptFxTintR, 137, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.ptFxTintG, 138, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.ptFxTintB, 139, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.ptFxSize, 144, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0)
                    );
                ...
                case MetaName.CExtensionDefAudioEmitter:
                    return new MetaStructureInfo(MetaName.CExtensionDefAudioEmitter, 15929839, 1024, 64,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetRotation, 32, MetaStructureEntryDataType.Float_XYZW,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.effectHash, 48, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0)
                    );
                case MetaName.CExtensionDefExplosionEffect:
                    return new MetaStructureInfo(MetaName.CExtensionDefExplosionEffect, 2840366784, 1024, 80,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetRotation, 32, MetaStructureEntryDataType.Float_XYZW,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.explosionName, 48, MetaStructureEntryDataType.CharPointer,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.boneTag, 64, MetaStructureEntryDataType.SignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.explosionTag, 68, MetaStructureEntryDataType.SignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.explosionType, 72, MetaStructureEntryDataType.SignedInt,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 76, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0)
                    );
                case MetaName.CExtensionDefLadder:
                    return new MetaStructureInfo(MetaName.CExtensionDefLadder, 1978210597, 1024, 96,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetPosition, 16, MetaStructureEntryDataType.Float_XYZ,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.bottom, 32, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.top, 48, MetaStructureEntryDataType.Float_XYZ, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.normal, 64, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.materialType, 80, MetaStructureEntryDataType.IntEnum, 0,
                            0, MetaName.CExtensionDefLadderMaterialType),
                        new MetaStructureEntryInfo_s(MetaName.template, 84, MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.canGetOffAtTop, 88, MetaStructureEntryDataType.Boolean, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.canGetOffAtBottom, 89, MetaStructureEntryDataType.Boolean,
                            0, 0, 0)
                    );
                ...
                case MetaName.CExtensionDefExpression:
                    return new MetaStructureInfo(MetaName.CExtensionDefExpression, 24441706, 1024, 48,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetPosition, 16, MetaStructureEntryDataType.Float_XYZ,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.expressionDictionaryName, 32,
                            MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.expressionName, 36, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.creatureMetadataName, 40, MetaStructureEntryDataType.Hash,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.initialiseOnCollision, 44,
                            MetaStructureEntryDataType.Boolean, 0, 0, 0)
                    );
                case MetaName.CExtensionDefLightShaft:
                    return new MetaStructureInfo(MetaName.CExtensionDefLightShaft, 2526429398, 1024, 176,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetPosition, 16, MetaStructureEntryDataType.Float_XYZ,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.cornerA, 32, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.cornerB, 48, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.cornerC, 64, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.cornerD, 80, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.direction, 96, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.directionAmount, 112, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.length, 116, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.fadeInTimeStart, 120, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.fadeInTimeEnd, 124, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.fadeOutTimeStart, 128, MetaStructureEntryDataType.Float,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.fadeOutTimeEnd, 132, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.fadeDistanceStart, 136, MetaStructureEntryDataType.Float,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.fadeDistanceEnd, 140, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.color, 144, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.intensity, 148, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.flashiness, 152, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 156, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.densityType, 160, MetaStructureEntryDataType.IntEnum, 0,
                            0, MetaName.CExtensionDefLightShaftDensityType),
                        new MetaStructureEntryInfo_s(MetaName.volumeType, 164, MetaStructureEntryDataType.IntEnum, 0, 0,
                            MetaName.CExtensionDefLightShaftVolumeType),
                        new MetaStructureEntryInfo_s(MetaName.softness, 168, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.scaleBySunIntensity, 172,
                            MetaStructureEntryDataType.Boolean, 0, 0, 0)
                    );
                ...
                case MetaName.CPedPropInfo:
                    return new MetaStructureInfo(MetaName.CPedPropInfo, 1792487819, 768, 40,
                        new MetaStructureEntryInfo_s(MetaName.numAvailProps, 0, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CPedPropMetaData),
                        new MetaStructureEntryInfo_s(MetaName.aPropMetaData, 8, MetaStructureEntryDataType.Array, 0, 1,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CAnchorProps),
                ...
                case MetaName.CPedVariationInfo:
                    return new MetaStructureInfo(MetaName.CPedVariationInfo, 4030871161, 768, 112,
                        new MetaStructureEntryInfo_s(MetaName.bHasTexVariations, 0, MetaStructureEntryDataType.Boolean,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.bHasDrawblVariations, 1,
                            MetaStructureEntryDataType.Boolean, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.bHasLowLODs, 2, MetaStructureEntryDataType.Boolean, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.bIsSuperLOD, 3, MetaStructureEntryDataType.Boolean, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.availComp, 4, MetaStructureEntryDataType.ArrayOfBytes, 0,
                            4, (MetaName)MetaTypeName.PsoPOINTER),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CPVComponentData),
                        new MetaStructureEntryInfo_s(MetaName.aComponentData3, 16, MetaStructureEntryDataType.Array, 0,
                            6, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CPedSelectionSet),
                        new MetaStructureEntryInfo_s(MetaName.aSelectionSets, 32, MetaStructureEntryDataType.Array, 0,
                            8, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CComponentInfo),
                        new MetaStructureEntryInfo_s(MetaName.compInfos, 48, MetaStructureEntryDataType.Array, 0, 10,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.propInfo, 64, MetaStructureEntryDataType.Structure, 0, 0,
                            MetaName.CPedPropInfo),
                ...
                case MetaName.CPVComponentData:
                    return new MetaStructureInfo(MetaName.CPVComponentData, 2024084511, 768, 24,
                        new MetaStructureEntryInfo_s(MetaName.numAvailTex, 0, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CPVDrawblData),
                        new MetaStructureEntryInfo_s(MetaName.aDrawblData3, 8, MetaStructureEntryDataType.Array, 0, 1,
                            0)
                    );
                ...
                case MetaName.CPVDrawblData:
                    return new MetaStructureInfo(MetaName.CPVDrawblData, 124073662, 768, 48,
                        new MetaStructureEntryInfo_s(MetaName.propMask, 0, MetaStructureEntryDataType.UnsignedByte, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.numAlternatives, 1,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CPVTextureData),
                        new MetaStructureEntryInfo_s(MetaName.aTexData, 8, MetaStructureEntryDataType.Array, 0, 2, 0),
                        new MetaStructureEntryInfo_s(MetaName.clothData, 24, MetaStructureEntryDataType.Structure, 0, 0,
                            MetaName.CPVDrawblData__CPVClothComponentData)
                    );
                case MetaName.CPVTextureData:
                    return new MetaStructureInfo(MetaName.CPVTextureData, 4272717794, 0, 3,
                        new MetaStructureEntryInfo_s(MetaName.texId, 0, MetaStructureEntryDataType.UnsignedByte, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.distribution, 1, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0)
                    );
                case MetaName.CComponentInfo:
                    return new MetaStructureInfo(MetaName.CComponentInfo, 3693847250, 512, 48,
                        new MetaStructureEntryInfo_s(MetaName.pedXml_audioID, 0, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.pedXml_audioID2, 4, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.pedXml_expressionMods, 8,
                            MetaStructureEntryDataType.ArrayOfBytes, 0, 2, (MetaName)5),
                        new MetaStructureEntryInfo_s(MetaName.flags, 28, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.inclusions, 32, MetaStructureEntryDataType.IntFlags2, 0,
                            32, 0),
                        new MetaStructureEntryInfo_s(MetaName.exclusions, 36, MetaStructureEntryDataType.IntFlags2, 0,
                            32, 0),
                        new MetaStructureEntryInfo_s(MetaName.pedXml_vfxComps, 40,
                            MetaStructureEntryDataType.ShortFlags, 0, 16, MetaName.ePedVarComp),
                        new MetaStructureEntryInfo_s(MetaName.pedXml_flags, 42,
                            MetaStructureEntryDataType.UnsignedShort, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.pedXml_compIdx, 44,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.pedXml_drawblIdx, 45,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0)
                    );
                case MetaName.CPedPropMetaData:
                    return new MetaStructureInfo(MetaName.CPedPropMetaData, 2029738350, 768, 56,
                        ...
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.expressionMods, 4,
                            MetaStructureEntryDataType.ArrayOfBytes, 0, 1, (MetaName)5),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.CPedPropTexData),
                        new MetaStructureEntryInfo_s(MetaName.texData, 24, MetaStructureEntryDataType.Array, 0, 3, 0),
                        new MetaStructureEntryInfo_s(MetaName.renderFlags, 40, MetaStructureEntryDataType.IntFlags1, 0,
                            3, MetaName.ePropRenderFlags),
                        new MetaStructureEntryInfo_s(MetaName.propFlags, 44, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 48, MetaStructureEntryDataType.UnsignedShort, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.anchorId, 50, MetaStructureEntryDataType.UnsignedByte, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.propId, 51, MetaStructureEntryDataType.UnsignedByte, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.stickyness, 52, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0)
                    );
                case MetaName.CPedPropTexData:
                    return new MetaStructureInfo(MetaName.CPedPropTexData, 2767296137, 512, 12,
                        new MetaStructureEntryInfo_s(MetaName.inclusions, 0, MetaStructureEntryDataType.IntFlags2, 0,
                            32, 0),
                        new MetaStructureEntryInfo_s(MetaName.exclusions, 4, MetaStructureEntryDataType.IntFlags2, 0,
                            32, 0),
                        new MetaStructureEntryInfo_s(MetaName.texId, 8, MetaStructureEntryDataType.UnsignedByte, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.inclusionId, 9, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.exclusionId, 10, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.distribution, 11, MetaStructureEntryDataType.UnsignedByte,
                            0, 0, 0)
                    );
                case MetaName.CAnchorProps:
                    return new MetaStructureInfo(MetaName.CAnchorProps, 403574180, 768, 24,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.anchor, 16, MetaStructureEntryDataType.IntEnum, 0, 0,
                            MetaName.eAnchorPoints)
                    );
                case MetaName.CPedSelectionSet:
                    return new MetaStructureInfo(MetaName.CPedSelectionSet, 3120284999, 512, 48,
                        ...
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.compDrawableId, 4,
                            MetaStructureEntryDataType.ArrayOfBytes, 0, 1, (MetaName)MetaTypeName.PsoPOINTER),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.compTexId, 16, MetaStructureEntryDataType.ArrayOfBytes, 0,
                            3, (MetaName)MetaTypeName.PsoPOINTER),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.propAnchorId, 28, MetaStructureEntryDataType.ArrayOfBytes,
                            0, 5, (MetaName)6),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.propDrawableId, 34,
                            MetaStructureEntryDataType.ArrayOfBytes, 0, 7, (MetaName)6),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.propTexId, 40, MetaStructureEntryDataType.ArrayOfBytes, 0,
                            9, (MetaName)6)
                    );
                case MetaName.CExtensionDefDoor:
                    return new MetaStructureInfo(MetaName.CExtensionDefDoor, 2671601385, 1024, 48,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetPosition, 16, MetaStructureEntryDataType.Float_XYZ,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.enableLimitAngle, 32, MetaStructureEntryDataType.Boolean,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.startsLocked, 33, MetaStructureEntryDataType.Boolean, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.canBreak, 34, MetaStructureEntryDataType.Boolean, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.limitAngle, 36, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.doorTargetRatio, 40, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                ...
                case MetaName.CExtensionDefSpawnPointOverride:
                    return new MetaStructureInfo(MetaName.CExtensionDefSpawnPointOverride, 2551875873, 1024, 64,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetPosition, 16, MetaStructureEntryDataType.Float_XYZ,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.ScenarioType, 32, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.iTimeStartOverride, 36,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.iTimeEndOverride, 37,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        ...
                        new MetaStructureEntryInfo_s(MetaName.AvailabilityInMpSp, 48,
                            MetaStructureEntryDataType.IntEnum, 0, 0, MetaName.CSpawnPoint__AvailabilityMpSp),
                        new MetaStructureEntryInfo_s(MetaName.Flags, 52, MetaStructureEntryDataType.IntFlags2, 0, 32,
                            MetaName.CScenarioPointFlags__Flags),
                        new MetaStructureEntryInfo_s(MetaName.Radius, 56, MetaStructureEntryDataType.Float, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.TimeTillPedLeaves, 60, MetaStructureEntryDataType.Float,
                            0, 0, 0)
                    );
                case MetaName.CExtensionDefWindDisturbance:
                    return new MetaStructureInfo(MetaName.CExtensionDefWindDisturbance, 3971538917, 1024, 96,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetRotation, 32, MetaStructureEntryDataType.Float_XYZW,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.disturbanceType, 48, MetaStructureEntryDataType.SignedInt,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.boneTag, 52, MetaStructureEntryDataType.SignedInt, 0, 0,
                            0),
                ...
                case MetaName.CCarGen:
                    return new MetaStructureInfo(MetaName.CCarGen, 2345238261, 1024, 80,
                        new MetaStructureEntryInfo_s(MetaName.position, 16, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        ...
                        new MetaStructureEntryInfo_s(MetaName.perpendicularLength, 40, MetaStructureEntryDataType.Float,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.carModel, 44, MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 48, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.bodyColorRemap1, 52, MetaStructureEntryDataType.SignedInt,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.bodyColorRemap2, 56, MetaStructureEntryDataType.SignedInt,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.bodyColorRemap3, 60, MetaStructureEntryDataType.SignedInt,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.bodyColorRemap4, 64, MetaStructureEntryDataType.SignedInt,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.popGroup, 68, MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.livery, 72, MetaStructureEntryDataType.SignedByte, 0, 0,
                            0)
                    );
                ...
                case MetaName.rage__fwGrassInstanceListDef:
                    return new MetaStructureInfo(MetaName.rage__fwGrassInstanceListDef, 941808164, 1024, 96,
                        new MetaStructureEntryInfo_s(MetaName.BatchAABB, 0, MetaStructureEntryDataType.Structure, 0, 0,
                            MetaName.rage__spdAABB),
                        new MetaStructureEntryInfo_s(MetaName.ScaleRange, 32, MetaStructureEntryDataType.Float_XYZ, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.archetypeName, 48, MetaStructureEntryDataType.Hash, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.lodDist, 52, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.LodFadeStartDist, 56, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.LodInstFadeRange, 60, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.OrientToTerrain, 64, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0,
                            MetaName.rage__fwGrassInstanceListDef__InstanceData),
                        new MetaStructureEntryInfo_s(MetaName.InstanceList, 72, MetaStructureEntryDataType.Array, 36, 7,
                            0)
                    );
                case MetaName.rage__fwGrassInstanceListDef__InstanceData:
                    return new MetaStructureInfo(MetaName.rage__fwGrassInstanceListDef__InstanceData, 2740378365, 256,
                        16,
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedShort, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.Position, 0, MetaStructureEntryDataType.ArrayOfBytes, 0,
                            0, (MetaName)3),
                        new MetaStructureEntryInfo_s(MetaName.NormalX, 6, MetaStructureEntryDataType.UnsignedByte, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.NormalY, 7, MetaStructureEntryDataType.UnsignedByte, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.Color, 8, MetaStructureEntryDataType.ArrayOfBytes, 0, 4,
                            (MetaName)3),
                        new MetaStructureEntryInfo_s(MetaName.Scale, 11, MetaStructureEntryDataType.UnsignedByte, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.Ao, 12, MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.UnsignedByte, 0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.Pad, 13, MetaStructureEntryDataType.ArrayOfBytes, 0, 8,
                            (MetaName)3)
                    );
                case MetaName.CExtensionDefProcObject:
                    return new MetaStructureInfo(MetaName.CExtensionDefProcObject, 3965391891, 1024, 80,
                        ...
                        new MetaStructureEntryInfo_s(MetaName.offsetPosition, 16, MetaStructureEntryDataType.Float_XYZ,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.radiusInner, 32, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.radiusOuter, 36, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        ...
                        new MetaStructureEntryInfo_s(MetaName.minZOffset, 60, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.maxZOffset, 64, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.objectHash, 68, MetaStructureEntryDataType.UnsignedInt, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.flags, 72, MetaStructureEntryDataType.UnsignedInt, 0, 0,
                            0)
                    );
                case MetaName.rage__phVerletClothCustomBounds:
                    return new MetaStructureInfo(MetaName.rage__phVerletClothCustomBounds, 2075461750, 768, 32,
                        new MetaStructureEntryInfo_s(MetaName.name, 8, MetaStructureEntryDataType.Hash, 0, 0, 0),
                        new MetaStructureEntryInfo_s((MetaName)MetaTypeName.ARRAYINFO, 0,
                            MetaStructureEntryDataType.Structure, 0, 0, MetaName.rage__phCapsuleBoundDef),
                        new MetaStructureEntryInfo_s(MetaName.CollisionData, 16, MetaStructureEntryDataType.Array, 0, 1,
                            0)
                    );
                case MetaName.rage__phCapsuleBoundDef:
                    return new MetaStructureInfo(MetaName.rage__phCapsuleBoundDef, 2859775340, 1024, 96,
                        new MetaStructureEntryInfo_s(MetaName.OwnerName, 0, MetaStructureEntryDataType.CharPointer, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.Rotation, 16, MetaStructureEntryDataType.Float_XYZW, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.Position, 32, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.Normal, 48, MetaStructureEntryDataType.Float_XYZ, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.CapsuleRadius, 64, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.CapsuleLen, 68, MetaStructureEntryDataType.Float, 0, 0,
                            0),
                        new MetaStructureEntryInfo_s(MetaName.CapsuleHalfHeight, 72, MetaStructureEntryDataType.Float,
                            0, 0, 0),
                        new MetaStructureEntryInfo_s(MetaName.CapsuleHalfWidth, 76, MetaStructureEntryDataType.Float, 0,
                            0, 0),
                        new MetaStructureEntryInfo_s(MetaName.Flags, 80, MetaStructureEntryDataType.IntFlags2, 0, 32,
                            MetaName.rage__phCapsuleBoundDef__enCollisionBoundDef)
                    );
        ...
        private static string GetSafeName(MetaName namehash, uint key)
        {
            ...
            if (string.IsNullOrEmpty(name)) name = "Unk_" + key;
            if (!char.IsLetter(name[0])) name = "Unk_" + name;
            return name;
        }
        ...
        public static byte[] ConvertArrayToBytes<T>(params T[] items) where T : struct
        {
            ...
            int size = Marshal.SizeOf(typeof(T)) * items.Length;
            byte[] b = new byte[size];
            GCHandle handle = GCHandle.Alloc(items, GCHandleType.Pinned);
            IntPtr h = handle.AddrOfPinnedObject();
        ...
        public static T ConvertData<T>(byte[] data) where T : struct
        {
            ...
            IntPtr h = handle.AddrOfPinnedObject();
            T r = Marshal.PtrToStructure<T>(h);
            handle.Free();
            return r;
        }
        ...
        public static T ConvertData<T>(byte[] data, int offset) where T : struct
        {
            GCHandle handle = GCHandle.Alloc(data, GCHandleType.Pinned);
            IntPtr h = handle.AddrOfPinnedObject();
            T r = Marshal.PtrToStructure<T>(h + offset);
            handle.Free();
            return r;
        }
        ...
        public static T[] ConvertDataArray<T>(Meta meta, MetaName name, Array_StructurePointer array) where T : struct
        {
            ...
            if (ptrs.Length < count) return null;
            ...
            for (int i = 0; i < count; i++)
            {
                MetaPOINTER ptr = ptrs[i];
                int offset = ptr.Offset;
                MetaDataBlock block = meta.GetBlock(ptr.BlockID);
                ...
                if (block.StructureNameHash != name) return null; //type mismatch - don't return anything...

                if (offset < 0 || block.Data == null || offset >= block.Data.Length) continue;
        ...
        public static T[] ConvertDataArray<T>(Meta meta, MetaName name, ulong pointer, uint count) where T : struct
        {
            ...
            MetaDataBlock ptrblock = ptrindex < meta.DataBlocks.Count ? meta.DataBlocks[(int)ptrindex] : null;
            if (ptrblock == null || ptrblock.Data == null ||
                ptrblock.StructureNameHash != name) return null; //no block or wrong block? shouldn't happen!
            ...
            while (itemsleft > 0)
            {
                ...
                if (itemcount > itemsleft) itemcount = itemsleft; //don't try to read too many items..
                ...
                curi += itemcount;
                itemsleft -= itemcount;
                if (itemsleft <= 0) return items; //all done!
                ptrindex++;
                ptrblock = ptrindex < meta.DataBlocks.Count ? meta.DataBlocks[(int)ptrindex] : null;
                if (ptrblock == null || ptrblock.Data == null) break; //not enough items..?

                if (ptrblock.StructureNameHash != name) break; //type mismatch..
            }
        ...
        public static MetaPOINTER[] GetPointerArray(Meta meta, Array_StructurePointer array)
        {
            ...
            MetaDataBlock ptrblock = meta.GetBlock((int)array.PointerDataId);
            if (ptrblock == null || ptrblock.Data == null ||
                ptrblock.StructureNameHash != (MetaName)MetaTypeName.POINTER) return null;

            for (int i = 0; i < count; i++)
            {
                int offset = ptroffset + i * ptrsize;
                if (offset >= ptrblock.Data.Length) break;
        ...
        public static byte[] GetByteArray(Meta meta, Array_byte array)
        {
            ...
            ushort count = array.Count1;
            if (ptroffset + count > ptrblock.Data.Length) return null;
        ...
        public static T[] GetTypedPointerArray<T>(Meta meta, MetaName name, MetaPOINTER[] arr) where T : struct
        {
            ...
            List<MetaDataBlock> datablocks = meta.DataBlocks.Data;
            if (datablocks == null) return null;
            int tsize = Marshal.SizeOf(typeof(T));
            List<T> list = new List<T>();
            for (int i = 0; i < arr.Length; i++)
            {
                ref MetaPOINTER ptr = ref arr[i];
                int blockind = ptr.BlockIndex;
                if (blockind < 0) continue;
                if (blockind >= datablocks.Count) continue;
                MetaDataBlock block = datablocks[blockind];
                if (block == null) continue;
                ...
                T item = ConvertData<T>(block.Data, offset);
                list.Add(item);
            }
        ...
        {
            foreach (MetaDataBlock block in meta.DataBlocks)
        ...
        public static string[] GetStrings(Meta meta)
        {
            ...
            {
                MetaDataBlock block = datablocks[i];
            ...
            if (startblock == null) return null; //couldn't find the strings data section.

            List<string> strings = new List<string>();
            StringBuilder sb = new StringBuilder();
            MetaDataBlock currentblock = startblock;
            ...
            if (strings.Count <= 0) return null; //don't return empty array...
            return strings.ToArray();
        }

        public static string GetString(Meta meta, CharPointer ptr)
        {
            int blocki = (int)ptr.PointerDataIndex; // (ptr.Pointer & 0xFFF) - 1;
            int offset = (int)ptr.PointerDataOffset; // (ptr.Pointer >> 12) & 0xFFFFF;
            if (blocki < 0 || blocki >= meta.DataBlocks.BlockLength) return null;
            MetaDataBlock block = meta.DataBlocks[blocki];
            if (block.StructureNameHash != (MetaName)MetaTypeName.STRING) return null;
            //var byteoffset = offset * 16 + offset2;
            ushort length = ptr.Count1;
            int lastbyte = offset + length;
            if (lastbyte >= block.DataLength) return null;
        ...
        public static MetaWrapper[] GetExtensions(Meta meta, Array_StructurePointer ptr)
        {
            ...
            MetaWrapper[] result = new MetaWrapper[ptr.Count1];
            MetaPOINTER[] extptrs = GetPointerArray(meta, ptr);
            if (extptrs != null)
                for (int i = 0; i < extptrs.Length; i++)
                {
                    MetaPOINTER extptr = extptrs[i];
                    MetaWrapper ext = null;
                    MetaDataBlock block = meta.GetBlock(extptr.BlockID);
                    MetaName h = block.StructureNameHash;
                    switch (h)
                    {
                        //archetype extension types
                        case MetaName.CExtensionDefParticleEffect:
                            ext =
                                new MCExtensionDefParticleEffect(); // MetaExtension<CExtensionDefParticleEffect>(h, GetData<CExtensionDefParticleEffect>(block, extptr));
                            break;
                        case MetaName.CExtensionDefAudioCollisionSettings:
                            ext =
                                new MCExtensionDefAudioCollisionSettings(); // MetaExtension<CExtensionDefAudioCollisionSettings>(h, GetData<CExtensionDefAudioCollisionSettings>(block, extptr));
                            break;
                        case MetaName.CExtensionDefAudioEmitter:
                            ext =
                                new MCExtensionDefAudioEmitter(); // MetaExtension<CExtensionDefAudioEmitter>(h, GetData<CExtensionDefAudioEmitter>(block, extptr));
                            break;
                        case MetaName.CExtensionDefSpawnPoint:
                            ext =
                                new MCExtensionDefSpawnPoint(); // new MetaExtension<CExtensionDefSpawnPoint>(h, GetData<CExtensionDefSpawnPoint>(block, extptr));
                            break;
                        case MetaName.CExtensionDefExplosionEffect:
                            ext =
                                new MCExtensionDefExplosionEffect(); // MetaExtension<CExtensionDefExplosionEffect>(h, GetData<CExtensionDefExplosionEffect>(block, extptr));
                            break;
                        case MetaName.CExtensionDefLadder:
                            ext =
                                new MCExtensionDefLadder(); // MetaExtension<CExtensionDefLadder>(h, GetData<CExtensionDefLadder>(block, extptr));
                            break;
                        case MetaName.CExtensionDefBuoyancy:
                            ext =
                                new MCExtensionDefBuoyancy(); // MetaExtension<CExtensionDefBuoyancy>(h, GetData<CExtensionDefBuoyancy>(block, extptr));
                            break;
                        case MetaName.CExtensionDefExpression:
                            ext =
                                new MCExtensionDefExpression(); // MetaExtension<CExtensionDefExpression>(h, GetData<CExtensionDefExpression>(block, extptr));
                            break;
                        case MetaName.CExtensionDefLightShaft:
                            ext =
                                new MCExtensionDefLightShaft(); // MetaExtension<CExtensionDefLightShaft>(h, GetData<CExtensionDefLightShaft>(block, extptr));
                            break;
                        case MetaName.CExtensionDefWindDisturbance:
                            ext =
                                new MCExtensionDefWindDisturbance(); // MetaExtension<CExtensionDefWindDisturbance>(h, GetData<CExtensionDefWindDisturbance>(block, extptr));
                            break;
                        case MetaName.CExtensionDefProcObject:
                            ext =
                                new MCExtensionDefProcObject(); // MetaExtension<CExtensionDefProcObject>(h, GetData<CExtensionDefProcObject>(block, extptr));
                            break;

                        //entity extension types
                        case MetaName.CExtensionDefLightEffect:
                            ext =
                                new MCExtensionDefLightEffect(); // MetaExtension<CExtensionDefLightEffect>(h, GetData<CExtensionDefLightEffect>(block, extptr));
                            break;
                        case MetaName.CExtensionDefSpawnPointOverride:
                            ext =
                                new MCExtensionDefSpawnPointOverride(); // MetaExtension<CExtensionDefSpawnPointOverride>(h, GetData<CExtensionDefSpawnPointOverride>(block, extptr));
                            break;
                        case MetaName.CExtensionDefDoor:
                        ...
                        case MetaName.rage__phVerletClothCustomBounds: //rage__phVerletClothCustomBounds
                            ext =
                                new Mrage__phVerletClothCustomBounds(); // MetaExtension<rage__phVerletClothCustomBounds>(h, GetData<rage__phVerletClothCustomBounds>(block, extptr));
                            break;
                    }

                    //string ts = GetTypesInitString(meta);
                    if (ext != null) ext.Load(meta, extptr);
                    if (i < result.Length) result[i] = ext;
                }
        ...
        public static int GetDataOffset(MetaDataBlock block, MetaPOINTER ptr)
        {
            ...
            int offset = ptr.Offset;
            if (offset < 0 || block.Data == null || offset >= block.Data.Length) return -1;
            return offset;
        }
        ...
        {
            MetaDataBlock block = meta.GetBlock(ptr.BlockID);
            int offset = GetDataOffset(block, ptr);
        ...
        {
            float x = SwapBytes(v.X);
            float y = SwapBytes(v.Y);
    ...
    [Flags]
    public enum CScenarioPointFlags__Flags //SCENARIO point flags  / extension spawn point flags
        //Key:2814596095
    {
    ...
    public struct CMapTypes //80 bytes, Key:2608875220
    {
        ...
        public Array_StructurePointer
            extensions { get; set; } //8   8: Array: 0: extensions  {0: StructurePointer: 0: 256}

        public Array_StructurePointer
            archetypes { get; set; } //24   24: Array: 0: archetypes  {0: StructurePointer: 0: 256}
            ...
            compositeEntityTypes
        {
            get;
            set;
        } //64   64: Array: 0: compositeEntityTypes  {0: Structure: SectionUNKNOWN2: 256}
    ...
    public struct CBaseArchetypeDef //144 bytes, Key:2411387556
    {
        ...
        public Array_StructurePointer
            extensions { get; set; } //120   120: Array: 0: extensions  {0: StructurePointer: 0: 256}
    ...
    public struct CBaseArchetypeDef_v2 //128 bytes, Key:2352343492  //old version...
    {
        ...
        public Array_StructurePointer
            extensions { get; set; } //112   112: Array: 0: extensions  {0: StructurePointer: 0: 256}
    }
    ...
    public struct CTimeArchetypeDef //160 bytes, Key:2520619910
    {
        ...
        public CBaseArchetypeDef BaseArchetypeDef
        {
            get => _BaseArchetypeDef;
            set => _BaseArchetypeDef = value;
        }

        public CTimeArchetypeDefData TimeArchetypeDef
        {
            get => _TimeArchetypeDef;
            set => _TimeArchetypeDef = value;
        }
    ...
    public struct CMloArchetypeDefData
    {
        ...
        public Array_StructurePointer
            entities { get; set; } //152   152: Array: 0: entities  {0: StructurePointer: 0: 256}

        public Array_Structure rooms { get; set; } //168   168: Array: 0: rooms  {0: Structure: CMloRoomDef: 256}

        public Array_Structure
            portals { get; set; } //184   184: Array: 0: portals//2314725778  {0: Structure: CMloPortalDef: 256}

        public Array_Structure
            entitySets { get; set; } //200   200: Array: 0: entitySets//1169996080  {0: Structure: CMloEntitySet: 256}
            ...
            timeCycleModifiers
        {
            get;
            set;
        } //216   216: Array: 0: timeCycleModifiers  {0: Structure: CMloTimeCycleModifier: 256}
    ...
    public struct CMloArchetypeDef //240 bytes, Key:937664754
    {
        ...
        public CMloArchetypeDefData MloArchetypeDef
        {
            get => _MloArchetypeDef;
            set => _MloArchetypeDef = value;
        }
    ...
    public struct CMloInstanceDef //160 bytes, Key:2151576752
    {
        ...
        public Array_uint
            defaultEntitySets { get; set; } //136   136: Array: 0: defaultEntitySets//1407157833  {0: Hash: 0: 256}
    ...
    public struct CMloRoomDef //112 bytes, Key:3885428245
    {
        ...
        public Array_uint
            attachedObjects { get; set; } //96   96: Array: 0: attachedObjects//2382704940  {0: UnsignedInt: 0: 256}
    }
    ...
    public class MCMloRoomDef : MetaWrapper
    {
        ...
        public CMloRoomDef Data => _Data;
        public string RoomName { get; set; }
        public uint[] AttachedObjects { get; set; }

        public Vector3 BBCenter => (_Data.bbMax + _Data.bbMin) * 0.5f;
        public Vector3 BBSize => _Data.bbMax - _Data.bbMin;
        public Vector3 BBMin => _Data.bbMin;
        public Vector3 BBMax => _Data.bbMax;
    ...
    public struct CMloPortalDef //64 bytes, Key:1110221513
    {
        ...
        public Array_uint
            attachedObjects { get; set; } //48   48: Array: 0: attachedObjects//2382704940  {0: UnsignedInt: 0: 256}
    }
    ...
    public class MCMloPortalDef : MetaWrapper
    {
        ...
        public Vector3 Center
        {
            get
            {
                ...
                for (int i = 0; i < Corners.Length; i++) v += Corners[i].XYZ();
    ...
    public struct CMloEntitySet //48 bytes, Key:4180211587
    {
        ...
        public Array_StructurePointer
            entities { get; set; } //32   32: Array: 0: entities  {0: StructurePointer: 0: 256}
    }
    ...
    public class MCMloEntitySet : MetaWrapper
    {
        ...
        public bool ForceVisible { get; set; } =
            false; //forces this entity set visible from the project window, for rendering  purpose
        ...
        private void Load(Meta meta)
        {
            ...
            CEntityDef[] ents = MetaTypes.ConvertDataArray<CEntityDef>(meta, MetaName.CEntityDef, _Data.entities);
            if (ents != null)
            {
        ...
        public override string ToString()
        {
            return Name + ": " + (Entities?.Length ?? 0) + " entities";
        }
    }
    ...
    public struct CMapData //512 bytes, Key:3448101671
    {
        ...
        public Array_StructurePointer
            entities { get; set; } //96   96: Array: 0: entities  {0: StructurePointer: 0: 256}

        public Array_Structure
            containerLods { get; set; } //112   112: Array: 0: containerLods//2935983381  {0: Structure: 372253349: 256}
            ...
            boxOccluders
        {
            get;
            set;
        } //128   128: Array: 0: boxOccluders//3983590932  {0: Structure: SectionUNKNOWN7: 256}
            ...
            occludeModels
        {
            get;
            set;
        } //144   144: Array: 0: occludeModels//2132383965  {0: Structure: SectionUNKNOWN5: 256}

        public Array_uint
            physicsDictionaries { get; set; } //160   160: Array: 0: physicsDictionaries//949589348  {0: Hash: 0: 256}

        public rage__fwInstancedMapData
            instancedData { get; set; } //176   176: Structure: rage__fwInstancedMapData: instancedData//2569067561
            ...
            timeCycleModifiers
        {
            get;
            set;
        } //224   224: Array: 0: timeCycleModifiers  {0: Structure: CTimeCycleModifier: 256}

        public Array_Structure
            carGenerators { get; set; } //240   240: Array: 0: carGenerators//3254823756  {0: Structure: CCarGen: 256}

        public CLODLight LODLightsSOA { get; set; } //256   256: Structure: CLODLight: LODLightsSOA//1774371066

        public CDistantLODLight
            DistantLODLightsSOA { get; set; } //392   392: Structure: CDistantLODLight: DistantLODLightsSOA//2954466641
    ...
    public struct CEntityDef //128 bytes, Key:1825799514
    {
        ...
        public Array_StructurePointer
            extensions { get; set; } //96   96: Array: 0: extensions  {0: StructurePointer: 0: 256}

        public int
            ambientOcclusionMultiplier { get; set; } //112   112: SignedInt: 0: ambientOcclusionMultiplier//415356295

        public int
            artificialAmbientOcclusion { get; set; } //116   116: SignedInt: 0: artificialAmbientOcclusion//599844163
        ...
        public override string ToString()
        {
            return JenkIndex.GetString(archetypeName) + ": " + JenkIndex.GetString(guid) + ": " + position;
        }
    }

    [TC(typeof(EXP))]
    public class MCEntityDef : MetaWrapper
    {
    ...
    public struct rage__fwInstancedMapData //48 bytes, Key:1836780118
    {
            ...
            PropInstanceList
        {
            get;
            set;
        } //16   16: Array: 0: PropInstanceList//3551474528  {0: Structure: rage__fwPropInstanceListDef: 256}
            ...
            GrassInstanceList
        {
            get;
            set;
        } //32   32: Array: 0: GrassInstanceList//255292381  {0: Structure: rage__fwGrassInstanceListDef: 256}

        public override string ToString()
        {
            return ImapLink + ", " + PropInstanceList.Count1 + " props, " + GrassInstanceList.Count1 + " grasses";
        }
    }
    ...
    public struct rage__fwGrassInstanceListDef //96 bytes, Key:941808164  rage__fwGrassInstanceListDef//2085051229
    {
            ...
            InstanceList
        {
            get;
            set;
        } //72   72: Array: 0: InstanceList//470289337  {0: Structure: rage__fwGrassInstanceListDef__InstanceData: 256}
        ...
        public override string ToString()
        {
            return archetypeName + " (" + InstanceList.Count1 + " instances)";
        }
    }

    [TC(typeof(EXP))]
    public struct
        rage__fwGrassInstanceListDef__InstanceData //16 bytes, Key:2740378365 rage__fwGrassInstanceListDef__InstanceData//3985044770 // Tom: Something to do with placing foliage
    {
    ...
    public struct CTimeCycleModifier //64 bytes, Key:2683420777
    {
        ...
        public override string ToString()
        {
            return name + ": startHour " + startHour + ", endHour " + endHour + ", range " + range + ", percentage " +
                   percentage;
        }
    }
    ...
    public struct CCarGen //80 bytes, Key:2345238261
    {
            ...
            flags
        {
            get;
            set;
        } //48   48: UnsignedInt: 0: flags   ///  _CP_: looks like flag 1879051873 in cargens forces to spawn a vehicle
        ...
        public override string ToString()
        {
            return carModel + ", " + position + ", " + popGroup + ", " + livery;
        }
    }
    ...
    public struct CLODLight //136 bytes, Key:2325189228
    {
        ...
        public Array_uint
            timeAndStateFlags { get; set; } //56   56: Array: 0: timeAndStateFlags=3112418278  {0: UnsignedInt: 0: 256}

        public Array_uint hash { get; set; } //72   72: Array: 0: hash  {0: UnsignedInt: 0: 256}

        public Array_byte
            coneInnerAngle { get; set; } //88   88: Array: 0: coneInnerAngle//1163671864  {0: UnsignedByte: 0: 256}
            ...
            coneOuterAngleOrCapExt
        {
            get;
            set;
        } //104   104: Array: 0: coneOuterAngleOrCapExt=3161894080  {0: UnsignedByte: 0: 256}

        public Array_byte
            coronaIntensity { get; set; } //120   120: Array: 0: coronaIntensity//2292363771  {0: UnsignedByte: 0: 256}
    }
    ...
    public struct CExtensionDefParticleEffect //96 bytes, Key:466596385
    {
        ...
        public override string ToString()
        {
            return "CExtensionDefParticleEffect - " + name;
        }
    }
    ...
    public class MCExtensionDefParticleEffect : MetaWrapper
    {
        ...
        public override MetaPOINTER Save(MetaBuilder mb)
        {
            if (fxName != null) _Data.fxName = mb.AddStringPtr(fxName);
    ...
    public struct CExtensionDefLightEffect //48 bytes, Key:2436199897
    {
        ...
        public Array_Structure
            instances { get; set; } //32   32: Array: 0: 274177522  {0: Structure: CLightAttrDef: 256}

        public override string ToString()
        {
            return "CExtensionDefLightEffect - " + name;
        }
    }

    [TC(typeof(EXP))]
    public class MCExtensionDefLightEffect : MetaWrapper
    {
        public CExtensionDefLightEffect _Data;
        public CExtensionDefLightEffect Data => _Data;
        ...
        public override MetaPOINTER Save(MetaBuilder mb)
        {
            if (instances != null) _Data.instances = mb.AddItemArrayPtr(MetaName.CLightAttrDef, instances);
        ...
        public override string ToString()
        {
            return _Data + " (" + (instances?.Length ?? 0) + " Attributes)";
        }
    }
    ...
    public struct CLightAttrDef //160 bytes, Key:2363260268
    {
            ...
            cullingPlane
        {
            get;
            set;
        } //48   48: ArrayOfBytes: 4: 1689591312 //48: 1689591312//cullingPlane - actually ArrayOfFloats4!
        ...
        public Vector3
            direction { get; set; } //112   112: ArrayOfBytes: 3: direction //actually ArrayOfFloats3 (Vector3!)
        ...
        public Vector3
            extents { get; set; } //144   144: ArrayOfBytes: 3: extents//759134656 //actually ArrayOfFloats3 (Vector3!)
    ...
    public struct CExtensionDefAudioCollisionSettings //48 bytes, Key:2701897500
    {
        ...
        public override string ToString()
        {
            return "CExtensionDefAudioCollisionSettings - " + name;
        }
    }

    [TC(typeof(EXP))]
    public class MCExtensionDefAudioCollisionSettings : MetaWrapper
    {
    ...
    public struct CExtensionDefAudioEmitter //64 bytes, Key:15929839
    {
        ...
        public override string ToString()
        {
            return "CExtensionDefAudioEmitter - " + name + ": " + effectHash + ": " + offsetPosition;
        }
    }
    ...
    public struct CExtensionDefExplosionEffect //80 bytes, Key:2840366784
    {
        ...
        public override string ToString()
        {
            return "CExtensionDefExplosionEffect - " + name;
        }
    }

    [TC(typeof(EXP))]
    public class MCExtensionDefExplosionEffect : MetaWrapper
    {
        ...
        public override MetaPOINTER Save(MetaBuilder mb)
        {
            if (explosionName != null) _Data.explosionName = mb.AddStringPtr(explosionName);
    ...
    public struct CExtensionDefLadder //96 bytes, Key:1978210597
    {
        ...
        public CExtensionDefLadderMaterialType
            materialType { get; set; } //80   80: IntEnum: CExtensionDefLadderMaterialType: materialType
        ...
        public override string ToString()
        {
            return "CExtensionDefLadder - " + name;
        }
    }
    ...
    [TC(typeof(EXP))]
    public class MCExtensionDefBuoyancy : MetaWrapper
    {
    ...
    public struct CExtensionDefExpression //48 bytes, Key:24441706
    {
        ...
        public override string ToString()
        {
            return "CExtensionDefExpression - " + name;
        }
    }
    ...
    public class MCExtensionDefExpression : MetaWrapper
    {
        ...
        public CExtensionDefExpression Data => _Data;
    ...
    public struct CExtensionDefLightShaft //176 bytes, Key:2526429398
    {
        ...
        public CExtensionDefLightShaftDensityType
            densityType { get; set; } //160   160: IntEnum: CExtensionDefLightShaftDensityType: densityType

        public CExtensionDefLightShaftVolumeType
            volumeType { get; set; } //164   164: IntEnum: CExtensionDefLightShaftVolumeType: volumeType
    ...
    public struct CExtensionDefSpawnPoint //96 bytes, Key:3077340721
    {
        ...
        public CSpawnPoint__AvailabilityMpSp
            availableInMpSp { get; set; } //68   68: IntEnum: CSpawnPoint__AvailabilityMpSp: availableInMpSp
        ...
        public override string ToString()
        {
            return
                spawnType + ": " + name + ", " +
                pedType; // + ", " + flags.ToString() + ", " + offsetPosition.ToString();
        }
    }
    ...
    public class MCExtensionDefSpawnPoint : MetaWrapper
    {
        ...
        public MCExtensionDefSpawnPoint(MCScenarioPointRegion region, Meta meta, CExtensionDefSpawnPoint data,
            object parent)
        {
        ...
        public CExtensionDefSpawnPoint Data => _Data;

        public Vector3 ParentPosition { get; set; } = Vector3.Zero;

        public Vector3 OffsetPosition
        {
            get => _Data.offsetPosition;
            set => _Data.offsetPosition = value;
        }

        public Vector4 OffsetRotation
        {
            get => _Data.offsetRotation;
            set => _Data.offsetRotation = value;
        }
        ...
        {
            get => _Data.spawnType;
            set => _Data.spawnType = value;
        }

        public MetaHash PedType
        {
            get => _Data.pedType;
            set => _Data.pedType = value;
        }
        ...
        {
            get => _Data.interior;
            set => _Data.interior = value;
        }

        public MetaHash RequiredImap
        {
            get => _Data.requiredImap;
            set => _Data.requiredImap = value;
        }

        public CSpawnPoint__AvailabilityMpSp AvailableInMpSp
        {
            get => _Data.availableInMpSp;
            set => _Data.availableInMpSp = value;
        }

        public float Probability
        {
            get => _Data.probability;
            set => _Data.probability = value;
        }

        public float TimeTillPedLeaves
        {
            get => _Data.timeTillPedLeaves;
            set => _Data.timeTillPedLeaves = value;
        }
        ...
        {
            get => _Data.highPri == 1;
            set => _Data.highPri = (byte)(value ? 1 : 0);
        }

        public bool ExtendedRange
        {
            get => _Data.extendedRange == 1;
            set => _Data.extendedRange = (byte)(value ? 1 : 0);
        }

        public bool ShortRange
        {
            get => _Data.shortRange == 1;
            set => _Data.shortRange = (byte)(value ? 1 : 0);
        }

        public Vector3 Position
        {
            get => _Data.offsetPosition + ParentPosition;
            set => _Data.offsetPosition = value - ParentPosition;
        }

        public Quaternion Orientation
        {
            get => new Quaternion(_Data.offsetRotation);
            set => _Data.offsetRotation = value.ToVector4();
        }
    ...
    public struct CExtensionDefSpawnPointOverride //64 bytes, Key:2551875873
    {
            ...
            AvailabilityInMpSp
        {
            get;
            set;
        } //48   48: IntEnum: CSpawnPoint__AvailabilityMpSp: AvailabilityInMpSp//2932681318
        ...
        public override string ToString()
        {
            return "CExtensionDefSpawnPointOverride - " + name;
        }
    }
    ...
    public struct CExtensionDefWindDisturbance //96 bytes, Key:3971538917
    {
        ...
        public override string ToString()
        {
            return "CExtensionDefWindDisturbance - " + name;
        }
    }
    ...
    public struct CExtensionDefProcObject //80 bytes, Key:3965391891
    {
        ...
        public override string ToString()
        {
            return "CExtensionDefProcObject - " + name;
        }
    }

    [TC(typeof(EXP))]
    public class MCExtensionDefProcObject : MetaWrapper
    {
        public CExtensionDefProcObject _Data;
        public CExtensionDefProcObject Data => _Data;
    ...
    public struct rage__phVerletClothCustomBounds //32 bytes, Key:2075461750
    {
        ...
        public Array_Structure
            CollisionData { get; set; } //16   16: Array: 0: CollisionData  {0: Structure: SectionUNKNOWN1: 256}
    }

    [TC(typeof(EXP))]
    public class Mrage__phVerletClothCustomBounds : MetaWrapper
    {
        public rage__phVerletClothCustomBounds _Data;
        public rage__phVerletClothCustomBounds Data => _Data;
        ...
        public override void Load(Meta meta, MetaPOINTER ptr)
        {
            ...
            rage__phCapsuleBoundDef[] cdata =
                MetaTypes.ConvertDataArray<rage__phCapsuleBoundDef>(meta, MetaName.rage__phCapsuleBoundDef,
                    _Data.CollisionData);
            if (cdata != null)
            {
                CollisionData = new Mrage__phCapsuleBoundDef[cdata.Length];
                for (int i = 0; i < cdata.Length; i++) CollisionData[i] = new Mrage__phCapsuleBoundDef(meta, cdata[i]);
            }
        }

        public override MetaPOINTER Save(MetaBuilder mb)
        {
            if (CollisionData != null) _Data.CollisionData = mb.AddWrapperArray(CollisionData);
        ...
        public override string ToString()
        {
            return "rage__phVerletClothCustomBounds - " + _Data.name + " (" + (CollisionData?.Length ?? 0) +
                   " CollisionData)";
        }
    }
    ...
    [TC(typeof(EXP))]
    public struct
        rage__phCapsuleBoundDef //96 bytes, Key:2859775340 //dexy: cloth CollisionData (child of rage__phVerletClothCustomBounds) ... eg josh house  // Tom: explosions? 
    {
        ...
        public rage__phCapsuleBoundDef__enCollisionBoundDef
            Flags { get; set; } //80   80: IntFlags2: rage__phCapsuleBoundDef__enCollisionBoundDef: Flags
    ...
    public class Mrage__phCapsuleBoundDef : MetaWrapper
    {
        ...
        public override MetaPOINTER Save(MetaBuilder mb)
        {
            if (OwnerName != null) _Data.OwnerName = mb.AddStringPtr(OwnerName);
    ...
    public struct rage__spdGrid2D //64 bytes, Key:894636096
    {
        ...
        public Vector2I Dimensions => new Vector2I(MaxCellX - MinCellX + 1, MaxCellY - MinCellY + 1);
    ...
    [TC(typeof(EXP))]
    public struct rage__spdSphere //16 bytes, Key:1189037266  //Sphere - used in scenario parts 
    {
    ...
    public struct CScenarioPointRegion //SCENARIO YMT ROOT  - in /scenario/ folder //376 bytes, Key:3501351821
    {
        ...
        public CScenarioPointContainer
            Points { get; set; } //8   8: Structure: CScenarioPointContainer//2380938603: Points//702683191
            ...
            EntityOverrides
        {
            get;
            set;
        } //72   72: Array: 0: EntityOverrides//697469539  {0: Structure: CScenarioEntityOverride//4213733800: 256}

        public uint Unused5 { get; set; } //88
        public uint Unused6 { get; set; } //92

        public CScenarioChainingGraph
            ChainingGraph { get; set; } //[PATHS] 96   96: Structure: CScenarioChainingGraph: ChainingGraph
            ...
            Clusters
        {
            get;
            set;
        } //264   264: Array: 0: Clusters//3587988394  {0: Structure: CScenarioPointCluster//750308016: 256}

        public CScenarioPointLookUps
            LookUps { get; set; } //280   280: Structure: CScenarioPointLookUps//3019621867: LookUps//1097626284
    }
    ...
    public class MCScenarioPointRegion : MetaWrapper
    {
        ...
        public CScenarioPointRegion Data => _Data;
        ...
        public ushort[]
            Unk_3844724227 { get; set; } //GRID DATA - 2d dimensions - AccelGrid ((MaxX-MinX)+1)*((MaxY-MinY)+1)
        ...
        public override void Load(Meta meta, MetaPOINTER ptr)
        {
            CScenarioPointRegion data = MetaTypes.GetData<CScenarioPointRegion>(meta, ptr);
            Load(meta, data);
        }
        ...
        public void Load(Meta meta, CScenarioPointRegion data)
        {
            ...
            CScenarioEntityOverride[] entOverrides =
                MetaTypes.ConvertDataArray<CScenarioEntityOverride>(meta, MetaName.CScenarioEntityOverride,
                    _Data.EntityOverrides);
            ...
            CScenarioPointCluster[] clusters =
                MetaTypes.ConvertDataArray<CScenarioPointCluster>(meta, MetaName.CScenarioPointCluster, _Data.Clusters);
            if (clusters != null)
            {
        ...
        public override MetaPOINTER Save(MetaBuilder mb)
        {
            MetaBuilderBlock sprb = mb.EnsureBlock(MetaName.CScenarioPointRegion);
            ...
            if (Points != null)
            {
                CScenarioPointContainer scp = new CScenarioPointContainer();

                CExtensionDefSpawnPoint[] loadSavePoints = Points.GetCLoadSavePoints();
                ...
                CScenarioPoint[] myPoints = Points.GetCMyPoints();
                if (myPoints != null)
                {
            ...
            if (EntityOverrides != null && EntityOverrides.Length > 0)
            {
                ...
                CScenarioEntityOverride[] cents = new CScenarioEntityOverride[EntityOverrides.Length];
                for (int i = 0; i < EntityOverrides.Length; i++)
                {
                    MCScenarioEntityOverride mcent = EntityOverrides[i];
                    CScenarioEntityOverride cent = mcent.Data;
                    CExtensionDefSpawnPoint[] scps = mcent.GetCScenarioPoints();
                    if (scps != null)
                    {
                        mb.AddStructureInfo(MetaName.CExtensionDefSpawnPoint);
                        mb.AddEnumInfo(MetaName.CSpawnPoint__AvailabilityMpSp);
            ...
            if (Paths != null)
            {
                CScenarioChainingGraph pd = new CScenarioChainingGraph();

                CScenarioChainingNode[] nodes = Paths.GetCNodes();
                if (nodes != null)
                {
                ...
                CScenarioChainingEdge[] edges = Paths.GetCEdges();
                if (edges != null)
                {
            ...
            if (LookUps != null)
            {
                CScenarioPointLookUps spl = new CScenarioPointLookUps();
            ...
            mb.AddString("Made with CodeWalker by dexyfex. " + DateTime.Now);
        ...
        public void AddCluster(MCScenarioPointCluster cluster)
        {
            ...
            if (Clusters != null) newclusters.AddRange(Clusters);
            cluster.Region = this;
        ...
        public void AddEntity(MCScenarioEntityOverride ent)
        {
            ...
            if (EntityOverrides != null) newents.AddRange(EntityOverrides);
            ent.Region = this;
        ...
        public bool RemoveCluster(MCScenarioPointCluster cluster)
        {
            ...
            {
                List<MCScenarioPointCluster> newclusters = new List<MCScenarioPointCluster>();
                foreach (MCScenarioPointCluster nc in Clusters)
                    if (nc == cluster)
                        r = true;
                    else
                ...
                if (r) Clusters = newclusters.ToArray();
            }
        ...
        public bool RemoveEntity(MCScenarioEntityOverride ent)
        {
            ...
            {
                List<MCScenarioEntityOverride> newents = new List<MCScenarioEntityOverride>();
                foreach (MCScenarioEntityOverride nc in EntityOverrides)
                    if (nc == ent)
                        r = true;
                    else
                ...
                if (r) EntityOverrides = newents.ToArray();
            }
    ...
    [TC(typeof(EXP))]
    public struct CScenarioPointContainer //SCENARIO Region Points arrays // 48 bytes, Key:2489654897
    {
        public Array_Structure
            LoadSavePoints
        {
            get;
            set;
        } //0   0: Array: 0: LoadSavePoints//3016741991  {0: Structure: CExtensionDefSpawnPoint: 256}
            ...
            MyPoints
        {
            get;
            set;
        } //16   16: Array: 0: MyPoints//1170781136  {0: Structure: CScenarioPoint//4103049490: 256}
    ...
    public class MCScenarioPointContainer : MetaWrapper
    {
        ...
        [TC(typeof(EXP))] public object Parent { get; set; }
        public MCScenarioPointRegion Region { get; }

        public CScenarioPointContainer Data
        {
            get => _Data;
            set => _Data = value;
        }
        ...
        {
            CExtensionDefSpawnPoint[] vLoadSavePoints =
                MetaTypes.ConvertDataArray<CExtensionDefSpawnPoint>(meta, MetaName.CExtensionDefSpawnPoint,
                    _Data.LoadSavePoints);
            ...
            CScenarioPoint[] vMyPoints =
                MetaTypes.ConvertDataArray<CScenarioPoint>(meta, MetaName.CScenarioPoint, _Data.MyPoints);
            if (vMyPoints != null)
            {
        ...
        public CExtensionDefSpawnPoint[] GetCLoadSavePoints()
        {
            ...
            for (int i = 0; i < LoadSavePoints.Length; i++) r[i] = LoadSavePoints[i].Data;
            return r;
        }
        ...
        public CScenarioPoint[] GetCMyPoints()
        {
            ...
            for (int i = 0; i < MyPoints.Length; i++) r[i] = MyPoints[i].Data;
            return r;
        }
        ...
        public void AddMyPoint(MCScenarioPoint p)
        {
            ...
            if (MyPoints != null) newpoints.AddRange(MyPoints);
        ...
        public void AddLoadSavePoint(MCExtensionDefSpawnPoint p)
        {
            ...
            if (LoadSavePoints != null) newpoints.AddRange(LoadSavePoints);
            newpoints.Add(p);
            p.Parent = this;
        ...
        public bool RemoveMyPoint(MCScenarioPoint p)
        {
            ...
            {
                List<MCScenarioPoint> newpoints = new List<MCScenarioPoint>();
                foreach (MCScenarioPoint mp in MyPoints)
                    if (mp == p)
                        r = true;
                    else
                        newpoints.Add(mp);
                ...
                if (r)
                {
                    ...
                    for (int i = 0; i < MyPoints.Length; i++) MyPoints[i].PointIndex = i;
                }
            }
        ...
        public bool RemoveLoadSavePoint(MCExtensionDefSpawnPoint p)
        {
            ...
            {
                List<MCExtensionDefSpawnPoint> newpoints = new List<MCExtensionDefSpawnPoint>();
                foreach (MCExtensionDefSpawnPoint mp in LoadSavePoints)
                    if (mp == p)
                        r = true;
                    else
                        newpoints.Add(mp);

                if (r) LoadSavePoints = newpoints.ToArray();
            }
    ...
    public struct CScenarioPoint //SCENARIO Point, similar to CExtensionDefSpawnPointOverride //64 bytes, Key:402442150
    {
        ...
        {
            return
                FloatUtil.GetVector4String(
                    vPositionAndDirection); //iTimeStartOverride.ToString() + "-" + iTimeEndOverride.ToString();// + ", " + Flags.ToString();
        }
    }
    ...
    public class MCScenarioPoint : MetaWrapper
    {
        ...
        public MCScenarioPoint(MCScenarioPointRegion region)
        {
            Region = region;
        }

        public MCScenarioPoint(MCScenarioPointRegion region, Meta meta, CScenarioPoint d,
            MCScenarioPointContainer container)
        {
            Region = region;
            Container = container;
            _Data = d;
        ...
        public Vector3 Position
        {
            get => _Data.vPositionAndDirection.XYZ();
            set => _Data.vPositionAndDirection = new Vector4(value, Direction);
        }

        public float Direction
        {
            get => _Data.vPositionAndDirection.W;
            set => _Data.vPositionAndDirection = new Vector4(Position, value);
        }
        ...
        {
            get => _Data.ModelSetId;
            set => _Data.ModelSetId = value;
        }
        ...
        {
            get => _Data.iScenarioGroup;
            set => _Data.iScenarioGroup = value;
        }
        ...
        public byte IMapId
        {
            get => _Data.iRequiredIMapId;
            set => _Data.iRequiredIMapId = value;
        }

        public MetaHash IMapName { get; set; }

        public string TimeRange => _Data.iTimeStartOverride.ToString().PadLeft(2, '0') + ":00 - " +
                                   _Data.iTimeEndOverride.ToString().PadLeft(2, '0') + ":00";

        public byte TimeStart
        {
            get => _Data.iTimeStartOverride;
            set => _Data.iTimeStartOverride = value;
        }

        public byte TimeEnd
        {
            get => _Data.iTimeEndOverride;
            set => _Data.iTimeEndOverride = value;
        }

        public byte Probability
        {
            get => _Data.iProbability;
            set => _Data.iProbability = value;
        }

        public byte AvailableMpSp
        {
            get => _Data.uAvailableInMpSp;
            set => _Data.uAvailableInMpSp = value;
        }

        public byte Radius
        {
            get => _Data.iRadius;
            set => _Data.iRadius = value;
        }

        public byte WaitTime
        {
            get => _Data.iTimeTillPedLeaves;
            set => _Data.iTimeTillPedLeaves = value;
        }
        ...
        public override string Name => (Type?.ToString() ?? "") + ": " + (ModelSet?.ToString() ?? "");
    ...
    public struct CScenarioEntityOverride //SCENARIO Entity Override //80 bytes, Key:1271200492
    {
            ...
            ScenarioPoints
        {
            get;
            set;
        } //24   24: Array: 0: ScenarioPoints  {0: Structure: CExtensionDefSpawnPoint: 256}
        ...
        public override string ToString()
        {
            return EntityType + ", " + ScenarioPoints.Count1 + " ScenarioPoints";
        }
    }
    ...
    public class MCScenarioEntityOverride : MetaWrapper
    {
        ...
        public CScenarioEntityOverride Data
        {
            get => _Data;
            set => _Data = value;
        }
        ...
        public Vector3 Position
        {
            get => _Data.EntityPosition;
            set => _Data.EntityPosition = value;
        }

        public MetaHash TypeName
        {
            get => _Data.EntityType;
            set => _Data.EntityType = value;
        }

        public bool EntityMayNotAlwaysExist
        {
            get => _Data.EntityMayNotAlwaysExist == 1;
            set => _Data.EntityMayNotAlwaysExist = (byte)(value ? 1 : 0);
        }

        public bool SpecificallyPreventArtPoints
        {
            get => _Data.SpecificallyPreventArtPoints == 1;
            set => _Data.SpecificallyPreventArtPoints = (byte)(value ? 1 : 0);
        }
        ...
        public void Init(Meta meta)
        {
            CExtensionDefSpawnPoint[] scenarioPoints =
                MetaTypes.ConvertDataArray<CExtensionDefSpawnPoint>(meta, MetaName.CExtensionDefSpawnPoint,
                    _Data.ScenarioPoints);
        ...
        {
            List<MCExtensionDefSpawnPoint> newpoints = new List<MCExtensionDefSpawnPoint>();
            if (ScenarioPoints != null) newpoints.AddRange(ScenarioPoints);
            newpoints.Add(p);
            p.Parent = this;
        ...
        public bool RemoveScenarioPoint(MCExtensionDefSpawnPoint p)
        {
            ...
            {
                List<MCExtensionDefSpawnPoint> newpoints = new List<MCExtensionDefSpawnPoint>();
                foreach (MCExtensionDefSpawnPoint mp in ScenarioPoints)
                    if (mp == p)
                        r = true;
                    else
                        newpoints.Add(mp);

                if (r) ScenarioPoints = newpoints.ToArray();
            }
        ...
        public CExtensionDefSpawnPoint[] GetCScenarioPoints()
        {
            ...
            for (int i = 0; i < ScenarioPoints.Length; i++) r[i] = ScenarioPoints[i].Data;
            return r;
        }
    ...
    [TC(typeof(EXP))]
    public struct CScenarioChainingGraph //SCENARIO PATH ARRAYS //88 bytes, Key:88255871
    {
        public Array_Structure
            Nodes { get; set; } //0   0: Array: 0: Nodes  {0: Structure: CScenarioChainingNode//3340683255: 256}

        public Array_Structure
            Edges { get; set; } //16   16: Array: 0: Edges  {0: Structure: CScenarioChainingEdge//4255409560: 256}
    ...
    public class MCScenarioChainingGraph : MetaWrapper
    {
        ...
        public MCScenarioChainingGraph(MCScenarioPointRegion region)
        {
            Region = region;
        }
        ...
        public CScenarioChainingGraph Data
        {
            get => _Data;
            set => _Data = value;
        }
        ...
        public override string Name => "CScenarioChainingGraph " + _Data;

        public void Init(Meta meta)
        {
            CScenarioChainingNode[] pathnodes =
                MetaTypes.ConvertDataArray<CScenarioChainingNode>(meta, MetaName.CScenarioChainingNode, _Data.Nodes);
            if (pathnodes != null)
            {
            ...
            CScenarioChainingEdge[] pathedges =
                MetaTypes.ConvertDataArray<CScenarioChainingEdge>(meta, MetaName.CScenarioChainingEdge, _Data.Edges);
            if (pathedges != null)
            {
            ...
            CScenarioChain[] pathchains =
                MetaTypes.ConvertDataArray<CScenarioChain>(meta, MetaName.CScenarioChain, _Data.Chains);
            if (pathchains != null)
            {
        ...
        public CScenarioChainingNode[] GetCNodes()
        {
            ...
            for (int i = 0; i < Nodes.Length; i++) r[i] = Nodes[i].Data;
            return r;
        }
        ...
        public CScenarioChainingEdge[] GetCEdges()
        {
            ...
            for (int i = 0; i < Edges.Length; i++) r[i] = Edges[i].Data;
            return r;
        }
        ...
        public CScenarioChain[] GetCChains()
        {
            ...
            for (int i = 0; i < Chains.Length; i++) r[i] = Chains[i].Data;
            return r;
        }
        ...
        public void AddNode(MCScenarioChainingNode node)
        {
            ...
            if (Nodes != null) newnodes.AddRange(Nodes);
            node.Parent = this;
            node.Region = Region;
        ...
        public void AddEdge(MCScenarioChainingEdge edge)
        {
            ...
            if (Edges != null) newedges.AddRange(Edges);
            edge.Region = Region;
        ...
        public void AddChain(MCScenarioChain chain)
        {
            ...
            if (Chains != null) newchains.AddRange(Chains);
            chain.Region = Region;
        ...
        public bool RemoveNode(MCScenarioChainingNode n)
        {
            ...
            if (Edges != null)
            {
                ...
                foreach (MCScenarioChainingEdge edge in Edges)
                    if (edge.NodeFrom == n || edge.NodeTo == n)
                        remedges.Add(edge);

                foreach (MCScenarioChainingEdge edge in remedges) RemoveEdge(edge);
            }

            if (Nodes != null)
            {
                List<MCScenarioChainingNode> newnodes = new List<MCScenarioChainingNode>();
                foreach (MCScenarioChainingNode nn in Nodes)
                    if (nn == n)
                    {
                        r = true;
                    }
                ...
                if (r)
                {
                    ...
                    foreach (MCScenarioChainingEdge e in Edges)
                    {
        ...
        public bool RemoveEdge(MCScenarioChainingEdge e)
        {
            ...
            {
                List<MCScenarioChainingEdge> newedges = new List<MCScenarioChainingEdge>();
                foreach (MCScenarioChainingEdge ne in Edges)
                    if (ne == e)
                    {
                        r = true;
                    }
                ...
                if (r)
                {
                    ...
                    List<MCScenarioChain> remchains = new List<MCScenarioChain>();
                    foreach (MCScenarioChain c in Chains)
                    {
                        if (c == null) continue;
                        if (c.RemoveEdge(e))
                    ...
                    foreach (MCScenarioChain c in remchains) RemoveChain(c);
                }
            }
        ...
        public bool RemoveChain(MCScenarioChain c)
        {
            ...
            {
                List<MCScenarioChain> newchains = new List<MCScenarioChain>();
                foreach (MCScenarioChain nc in Chains)
                    if (nc == c)
                    {
                        r = true;
                    }
    ...
    public struct CScenarioChainingNode //SCENARIO PATH NODE //32 bytes, Key:1811784424
    {
            ...
            Unk_2602393771
        {
            get;
            set;
        } //16   16: Hash: 0: 2602393771  prop name, eg. prop_parknmeter_01, prop_atm_01, prop_bench_01a
        ...
        {
            return
                ScenarioType + ", " + Unk_2602393771;
        }
    }

    [TC(typeof(EXP))]
    public class MCScenarioChainingNode : MetaWrapper
    {
        ...
        public MCScenarioChainingNode(MCScenarioPointRegion region, Meta meta, CScenarioChainingNode d,
            MCScenarioChainingGraph parent, int index)
        {
            Region = region;
            Parent = parent;
            _Data = d;
        ...
        public CScenarioChainingNode Data
        {
            get => _Data;
            set => _Data = value;
        }
        ...
        public MetaHash PropHash
        {
            get => _Data.Unk_2602393771;
            set => _Data.Unk_2602393771 = value;
        }

        public MetaHash TypeHash
        {
            get => _Data.ScenarioType;
            set => _Data.ScenarioType = value;
        }
        ...
        {
            get => _Data.HasIncomingEdges == 1;
            set => _Data.HasIncomingEdges = (byte)(value ? 1 : 0);
        }

        public bool HasOutgoingEdges
        {
            get => _Data.HasOutgoingEdges == 1;
            set => _Data.HasOutgoingEdges = (byte)(value ? 1 : 0);
        }

        public int NodeIndex { get; set; }
        public MCScenarioChain Chain { get; set; }

        public override string Name => "CScenarioChainingNode"; // + _Data.Unk_2602393771.ToString();
    ...
    [TC(typeof(EXP))]
    public struct CScenarioChainingEdge //SCENARIO PATH EDGE //8 bytes, Key:2004985940
    {
        ...
        public CScenarioChainingEdge__eAction
            Action { get; set; } //4   4: ByteEnum: CScenarioChainingEdge__eAction: Action

        public CScenarioChainingEdge__eNavMode
            NavMode { get; set; } //5   5: ByteEnum: CScenarioChainingEdge__eNavMode: NavMode

        public CScenarioChainingEdge__eNavSpeed
            NavSpeed { get; set; } //6   6: ByteEnum: CScenarioChainingEdge__eNavSpeed: NavSpeed
    ...
    [TC(typeof(EXP))]
    public class MCScenarioChainingEdge : MetaWrapper
    {
        ...
        public CScenarioChainingEdge Data
        {
            get => _Data;
            set => _Data = value;
        }
        ...
        {
            get => _Data.NodeIndexFrom;
            set => _Data.NodeIndexFrom = value;
        }

        public ushort NodeIndexTo
        {
            get => _Data.NodeIndexTo;
            set => _Data.NodeIndexTo = value;
        }

        public CScenarioChainingEdge__eAction Action
        {
            get => _Data.Action;
            set => _Data.Action = value;
        }

        public CScenarioChainingEdge__eNavMode NavMode
        {
            get => _Data.NavMode;
            set => _Data.NavMode = value;
        }

        public CScenarioChainingEdge__eNavSpeed NavSpeed
        {
            get => _Data.NavSpeed;
            set => _Data.NavSpeed = value;
        }
    ...
    [TC(typeof(EXP))]
    public struct CScenarioChain //SCENARIO PATH CHAIN //40 bytes, Key:2751910366
    {
    ...
    [TC(typeof(EXP))]
    public class MCScenarioChain : MetaWrapper
    {
        ...
        public byte Unk1
        {
            get => _Data.Unk_1156691834;
            set => _Data.Unk_1156691834 = value;
        }
        ...
        public override string Name => "CScenarioChain";
        ...
        public void AddEdge(MCScenarioChainingEdge edge)
        {
            ...
            if (Edges != null) newedges.AddRange(Edges);
            if (EdgeIds != null) newedgeids.AddRange(EdgeIds);
            edge.Region = Region;
            newedges.Add(edge);
    ...
    [TC(typeof(EXP))]
    public struct CScenarioPointCluster //SCENARIO spawn cluster - all things spawn together //80 bytes, Key:3622480419
    {
        public CScenarioPointContainer
            Points { get; set; } //0   0: Structure: CScenarioPointContainer//2380938603: Points//702683191
    ...
    [TC(typeof(EXP))]
    public class MCScenarioPointCluster : MetaWrapper
    {
        ...
        public MCScenarioPointCluster(MCScenarioPointRegion region)
        {
            Region = region;
        }
        ...
        public CScenarioPointCluster Data
        {
            get => _Data;
            set => _Data = value;
        }
        ...
        {
            get => _Data.ClusterSphere.centerAndRadius.XYZ();
            set
            {
                Vector4 v4 = new Vector4(value, _Data.ClusterSphere.centerAndRadius.W);
        ...
        public float Radius
        {
            get => _Data.ClusterSphere.centerAndRadius.W;
            set
            {
                Vector4 v4 = new Vector4(_Data.ClusterSphere.centerAndRadius.XYZ(), value);
                _Data.ClusterSphere = new rage__spdSphere { centerAndRadius = v4 };
            }
        }

        public float NextSpawnAttemptDelay
        {
            get => _Data.NextSpawnAttemptDelay;
            set => _Data.NextSpawnAttemptDelay = value;
        }

        public bool AllPointsRequiredForSpawn
        {
            get => _Data.AllPointsRequiredForSpawn == 1;
            set => _Data.AllPointsRequiredForSpawn = (byte)(value ? 1 : 0);
        }

        public override string Name => "CScenarioPointCluster";
    ...
    [TC(typeof(EXP))]
    public struct CScenarioPointLookUps //SCENARIO hash arrays //96 bytes, Key:2669361587
    {
        public Array_uint TypeNames { get; set; } //0   0: Array: 0: TypeNames//3057471271  {0: Hash: 0: 256}

        public Array_uint
            PedModelSetNames { get; set; } //16   16: Array: 0: PedModelSetNames//3020866217  {0: Hash: 0: 256}

        public Array_uint
            VehicleModelSetNames { get; set; } //32   32: Array: 0: VehicleModelSetNames//3827910541  {0: Hash: 0: 256}
            ...
            RequiredIMapNames
        {
            get;
            set;
        } //[ymap names] //80   80: Array: 0: RequiredIMapNames//1767860162  {0: Hash: 0: 256}
    ...
    [TC(typeof(EXP))]
    public class MCScenarioPointLookUps : MetaWrapper
    {
        ...
        public CScenarioPointLookUps Data
        {
            get => _Data;
            set => _Data = value;
        }
        ...
        public override string Name => "CScenarioPointLookUps";
    ...
    [TC(typeof(EXP))]
    public struct
        CCompositeEntityType //304 bytes, Key:659539004 dex: composite entity type - ytyp // Tom: des_ destruction
    {
        ...
        public Array_Structure
            Animations { get; set; } //280   280: Array: 0: Animations  {0: Structure: 1980345114: 256}
        ...
        public override string ToString()
        {
            return Name + ", " + StartModel + ", " + EndModel + ", " +
                   StartImapFile + ", " + EndImapFile + ", " + PtFxAssetName;
        }
    }

    [TC(typeof(EXP))]
    public struct CCompEntityAnims //216 bytes, Key:4110496011 //destruction animations?
    {
        ...
        public Array_Structure
            effectsData { get; set; } //200   200: Array: 0: effectsData  {0: Structure: 3430328684: 256}
    }
    ...
    {
        public Array_Structure
            Frames { get; set; } //0   0: Array: 0: Frames  {0: Structure: CStreamingRequestFrame: 256}

        public Array_Structure
            CommonSets { get; set; } //16   16: Array: 0: CommonSets  {0: Structure: CStreamingRequestCommonSet: 256}
    ...
    public struct CCreatureMetaData //56 bytes, Key:2181653572
    {
            ...
            shaderVariableComponents
        {
            get;
            set;
        } //8   8: Array: 0: shaderVariableComponents  {0: Structure: CShaderVariableComponent: 256}
            ...
            pedPropExpressions
        {
            get;
            set;
        } //24   24: Array: 0: pedPropExpressions  {0: Structure: CPedPropExpressionData: 256}
            ...
            pedCompExpressions
        {
            get;
            set;
        } //40   40: Array: 0: pedCompExpressions  {0: Structure: CPedCompExpressionData: 256}
    }
    ...
    [TC(typeof(EXP))]
    public struct
        CPedVariationInfo : IPsoSwapEnd //112 bytes, Key:4030871161  //COMPONENT PEDS YMT ROOT  - in componentpeds .rpf's
    {
        ...
        public Array_Structure
            aComponentData3 { get; set; } //16   16: Array: 0: aComponentData3  {0: Structure: CPVComponentData: 256}

        public Array_Structure
            aSelectionSets { get; set; } //32   32: Array: 0: aSelectionSets  {0: Structure: CPedSelectionSet: 256}

        public Array_Structure
            compInfos { get; set; } //48   48: Array: 0: compInfos  {0: Structure: CComponentInfo: 256}
    ...
    public class MCPedVariationInfo : MetaWrapper
    {
        ...
        public override void Load(Meta meta, MetaPOINTER ptr)
        {
            CPedVariationInfo data = MetaTypes.GetData<CPedVariationInfo>(meta, ptr);
            Load(meta, data);
        }
        ...
        public void Load(Meta meta, CPedVariationInfo data)
        {
            ...
            CPVComponentData[] aComponentData3 =
                MetaTypes.ConvertDataArray<CPVComponentData>(meta, MetaName.CPVComponentData, _Data.aComponentData3);
            ...
            CPedSelectionSet[] vSelectionSets =
                MetaTypes.ConvertDataArray<CPedSelectionSet>(meta, MetaName.CPedSelectionSet, _Data.aSelectionSets);
            ...
            CComponentInfo[] vCompInfos =
                MetaTypes.ConvertDataArray<CComponentInfo>(meta, MetaName.CComponentInfo, _Data.compInfos);
            if (vCompInfos != null)
            {
            ...
            {
                byte compInd = ComponentIndices[i];
                if (compInd > 0 && compInd < ComponentData3?.Length)
                {
                    MCPVComponentData compvar = ComponentData3[compInd];
                    compvar.ComponentType = i;

                    if (compvar.DrawblData3 != null)
                        foreach (MCPVDrawblData cvp in compvar.DrawblData3)
                            cvp.ComponentType = i;
        ...
        public MCPVComponentData GetComponentData(int componentType)
        {
            ...
            byte index = ComponentIndices[componentType];
    ...
    [TC(typeof(EXP))]
    public struct CPVComponentData //24 bytes, Key:2024084511  //COMPONENT PEDS component variations item
    {
        ...
        public Array_Structure
            aDrawblData3 { get; set; } //8   8: Array: 0: aDrawblData3  {0: Structure: CPVDrawblData: 256}
    }

    [TC(typeof(EXP))]
    public class MCPVComponentData : MetaWrapper
    {
        ...
        {
            get => _Data.numAvailTex;
            set => _Data.numAvailTex = value;
        }
        ...
        {
            CPVDrawblData[] aDrawblData3 =
                MetaTypes.ConvertDataArray<CPVDrawblData>(meta, MetaName.CPVDrawblData, _Data.aDrawblData3);
        ...
        public override string ToString()
        {
            ...
            return r + " : " + DrawblData3?.Length ?? base.ToString();
        }
    }

    [TC(typeof(EXP))]
    public struct CPVDrawblData //48 bytes, Key:124073662  //COMPONENT PEDS drawable info
    {
        ...
        public CPVDrawblData__CPVClothComponentData
            clothData { get; set; } //24   24: Structure: CPVDrawblData__CPVClothComponentData: clothData
    }

    [TC(typeof(EXP))]
    public class MCPVDrawblData : MetaWrapper
    {
        ...
        public string GetTextureSuffix(int texnum)
        {
            ...
            CPVTextureData tex = TexData[texnum];
            byte texid = tex.texId;
            byte distr = tex.distribution; //what does this do?
    ...
    [TC(typeof(EXP))]
    public struct CPVDrawblData__CPVClothComponentData //24 bytes, Key:508935687  //COMPONENT PEDS clothData
    {
    ...
    public class MCComponentInfo : MetaWrapper
    {
        ...
        public override string ToString()
        {
            return ComponentType < 12
                ? MCPVComponentData.ComponentTypeNames[ComponentType] + "_" + ComponentIndex.ToString("000")
                : base.ToString();
        }
    }
    ...
    public struct CPedPropInfo //40 bytes, Key:1792487819  //COMPONENT PEDS 
    {
        ...
        public Array_Structure
            aPropMetaData { get; set; } //8   8: Array: 0: aPropMetaData  {0: Structure: CPedPropMetaData: 256}
    ...
    public class MCPedPropInfo : MetaWrapper
    {
        ...
        public MCPedPropInfo(Meta meta, CPedPropInfo data, MCPedVariationInfo owner)
        {
            ...
            CPedPropMetaData[] vPropMetaData =
                MetaTypes.ConvertDataArray<CPedPropMetaData>(meta, MetaName.CPedPropMetaData, _Data.aPropMetaData);
            ...
            CAnchorProps[] vAnchors =
                MetaTypes.ConvertDataArray<CAnchorProps>(meta, MetaName.CAnchorProps, _Data.aAnchors);
            if (vAnchors != null)
            {
                Anchors = new MCAnchorProps[vAnchors.Length];
                for (int i = 0; i < vAnchors.Length; i++) Anchors[i] = new MCAnchorProps(meta, vAnchors[i], this);
            }
        }

...
namespace CodeWalker.World
{
    ...
    public class TrainTrack : BasePathData
    {
        ...
        public int StationCount
        {
            get
            {
                int sc = 0;
                if (Nodes != null)
                    foreach (TrainTrackNode node in Nodes)
        ...
        public byte[] Save()
        {
            ...
            foreach (TrainTrackNode node in Nodes)
            {
                string nstr = FloatUtil.GetVector3String(node.Position).Replace(",", "") + " " + node.NodeType;
                sb.AppendLine(nstr);
            }
        ...
        public void Load(string trackstr)
        {
            ...
            if (!string.IsNullOrEmpty(trackstr))
            {
                ...
                if (trackstrs.Length > 1)
                {
                    ...
                    {
                        string nodestr = trackstrs[i].Trim();
                        string[] nodevals = nodestr.Split(' ');
                        if (nodevals.Length == 4)
                        {
                            TrainTrackNode ttnode = new TrainTrackNode();
                            float x = FloatUtil.Parse(nodevals[0]);
                            float y = FloatUtil.Parse(nodevals[1]);
                            float z = FloatUtil.Parse(nodevals[2]);
                            int nodetype;
                            ...
                            if (ttnode.Links[0] != null) ttnode.Links[0].Links[1] = ttnode;
                            nodes.Add(ttnode);
                        }
                    }
            ...
            if (Nodes == null) Nodes = new List<TrainTrackNode>();
        }
        ...
        public void BuildVertices()
        {
            if (Nodes != null && Nodes.Count > 0)
            {
                ...
                int lc = nc - 1;
                int lvc = lc * 2;
                Vector4[] np = new Vector4[nc];
                EditorVertex[] lv = new EditorVertex[lvc];
                for (int i = 0; i < nc; i++)
                {
        ...
        public bool RemoveNode(TrainTrackNode node)
        {
            ...
            if (r)
            {
                TrainTrackNode l0 = node.Links[0];
                TrainTrackNode l1 = node.Links[1];

                if (l0 != null) l0.Links[1] = l1;
                if (l1 != null) l1.Links[0] = l0;
        ...
        {
            return Name + ": " + filename + " (" + NodeCount + " nodes)";
        }
    }
    ...
    public class TrainTrackNode : BasePathNode
    {
        ...
        public override string ToString()
        {
            return Index + ": " + NodeType; // + ": " + FloatUtil.GetVector3String(Position);
        }
    }
}
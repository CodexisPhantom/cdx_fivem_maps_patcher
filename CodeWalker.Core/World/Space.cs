...
namespace CodeWalker.World
{
    public class Space
    {
        ...
        private void InitManifestData()
        {
            ...
            List<YmfFile> manifests = GameFileCache.AllManifests;
            foreach (YmfFile manifest in manifests)
            {
        ...
        private void InitCacheData()
        {
            ...
            List<CacheDatFile> caches = GameFileCache.AllCacheFiles;
            ...
            foreach (CacheDatFile cache in caches)
            {
                foreach (CacheFileDate filedate in cache.FileDates)
                {
                ...
                foreach (CInteriorProxy intprx in cache.AllCInteriorProxies)
                {
        ...
        public void BuildYndLinks(YndFile ynd, List<YndLink> tlinks = null, List<YndLink> nlinks = null)
        {
            ...
            {
                nlinks.Clear();
                YndNode node = ynodes[i];

                ushort linkid = node.LinkID;
                for (int l = 0; l < node.LinkCount; l++)
                {
                    int llid = linkid + l;
                    if (llid >= links.Length) continue;
                    NodeLink link = links[llid];
                    YndNode tnode;
        ...
        public void BuildYndVerts(YndFile ynd, YndNode[] selectedNodes, List<EditorVertex> tverts = null)
        {
            ...
            for (int i = 0; i < nodecount; i++)
            {
                ...
                EditorVertex nvert = new EditorVertex();
                ...
                for (int l = 0; l < node.Links.Length; l++)
                {
                    ...
                    Vector3 laneDir = yl.GetDirection();
                    Vector3 laneDirCross = Vector3.Cross(laneDir, Vector3.UnitZ);
                    float laneWidth = yl.GetLaneWidth();
                    float laneHalfWidth = laneWidth / 2;
                    float offset = yl.IsTwoWay()
                        ? yl.LaneOffset * laneWidth - laneHalfWidth
                        : yl.LaneOffset - yl.LaneCountForward * laneWidth / 2f + laneHalfWidth;

                    int iOffset = yl.IsTwoWay() ? 1 : 0;

                    YndNode tnode = yl.Node2;

                    if (tnode == null) continue; //invalid links could hit here
                    EditorVertex tvert = new EditorVertex();
                    ...
                    for (int j = iOffset; j < yl.LaneCountForward + iOffset; j++)
                    {
                        ...
                        EditorVertex lvert2 = new EditorVertex
                        {
        ...
        public void BuildYndJuncs(YndFile ynd)
        {
            ...
            YndJunction[] yjuncs = ynd.Junctions;
            if (yjuncs != null)
            {
                int junccount = yjuncs.Length;
        ...
        {
            int xDir = Math.Min(1, Math.Max(-1, desiredX - ynd.CellX));
            int yDir = Math.Min(1, Math.Max(-1, desiredY - ynd.CellY));
            int x = desiredX;
            int y = desiredY;
            ...
            int dy = y - ynd.CellY;
            int areaId = y * 32 + x;
            int areaIdorig = ynd.AreaID;
            bool changed = ynd.AreaID != areaId;
            ynd.CellX = x;
            ynd.CellY = y;
            ynd.AreaID = areaId;
        ...
        public void Update(float elapsed)
        {
            ...
            foreach (Entity e in EnabledEntities)
            {
                ...
                {
                    SpaceEntityCollision coll = FindFirstCollision(e, elapsed);
        ...
        public SpaceEntityCollision FindFirstCollision(Entity e, float elapsed)
        {
            ...
            if (r.SphereHit.Hit)
            {
                ...
                while (curiter < maxiter) //iterate to find a closer hit time... improve this!
                {
                    ...
                    SpaceSphereIntersectResult tcollres = SphereIntersect(sph, CollisionLayers);
                    if (tcollres.Hit)
                    {
                        r.HitT = curt;
        ...
        public void GetVisibleYmaps(Camera cam, int hour, MetaHash weather, Dictionary<MetaHash, YmapFile> ymaps)
        {
            ...
            List<MapDataStoreNode> items = MapDataStore.GetItems(ref cam.Position);
        ...
        public void GetVisibleBounds(Camera cam, int gridrange, bool[] layers, List<BoundsStoreItem> boundslist)
        {
            ...
            List<BoundsStoreItem> items = BoundsStore.GetItems(ref min, ref max, layers);
        ...
        public SpaceRayIntersectResult RayIntersect(Ray ray, float maxdist = float.MaxValue, bool[] layers = null)
        {
            ...
            {
                BoundsStoreItem bound = boundslist[i];
                    ...
                    if ((ymap != null) && (ymap.Loaded) && (ymap.AllEntities != null))
                    {
                        ...
                        for (int e = 0; e < ymap.AllEntities.Length; e++)
                        {
                            ...
                            if (ray.Intersects(ref box, out boxhitdisttest))
                            {
                                ...
                                if (ent.IsMlo)
                                {
                                    SpaceRayIntersectResult ihit = RayIntersectInterior(ref ray, ent, res.HitDist);
                                    res.TryUpdate(ref ihit);
                                }
                                else
                                {
                                    SpaceRayIntersectResult ehit = RayIntersectEntity(ref ray, ent, res.HitDist);
        ...
        public SpaceRayIntersectResult RayIntersectEntity(ref Ray ray, YmapEntityDef ent, float maxdist = float.MaxValue)
        {
            ...
            DrawableBase drawable = GameFileCache.TryGetDrawable(ent.Archetype);
            if (drawable != null)
            {
                Quaternion eori = ent.Orientation;
                Quaternion eorinv = Quaternion.Invert(ent.Orientation);
                Ray eray = new Ray();
                ...
                else if (drawable is FragDrawable fdrawable)
                {
                    ...
                    {
                        SpaceRayIntersectResult fhit = fdrawable.Bound.RayIntersect(ref eray, res.HitDist);
                        if (fhit.Hit)
                        {
                    ...
                    if ((ymap != null) && (ymap.Loaded) && (ymap.AllEntities != null))
                    {
                        ...
                        for (int e = 0; e < ymap.AllEntities.Length; e++)
                        {
                            ...
                            if (sph.Intersects(ref box))
                            {
                                if (ent.IsMlo)
                                {
                                    SpaceSphereIntersectResult ihit = SphereIntersectInterior(ref sph, ent);
                                    res.TryUpdate(ref ihit);
                                }
                                else
                                {
                                    SpaceSphereIntersectResult ehit = SphereIntersectEntity(ref sph, ent);
                                    res.TryUpdate(ref ehit);
                                }
                            }
                        }
        ...
        public SpaceSphereIntersectResult SphereIntersectEntity(ref BoundingSphere sph, YmapEntityDef ent)
        {
            ...
            if (drawable != null)
            {
                ...
                else if (drawable is FragDrawable fdrawable)
                {
                    if (fdrawable.Bound != null)
                    {
                        SpaceSphereIntersectResult fhit = fdrawable.Bound.SphereIntersect(ref esph);
                        if (fhit.Hit)
                        {
                            fhit.Position = eori.Multiply(fhit.Position) + ent.Position;
                            fhit.Normal = eori.Multiply(fhit.Normal);
        ...
        public SpaceSphereIntersectResult SphereIntersectInterior(ref BoundingSphere sph, YmapEntityDef mlo)
        {
            ...
            if (mlodat.Entities != null)
            {
                for (int j = 0; j < mlodat.Entities.Length; j++) //should really improve this by using rooms!
                {
                    ...
                    if (sph.Intersects(ref box))
                    {
                        SpaceSphereIntersectResult ehit = SphereIntersectEntity(ref sph, intent);
                        res.TryUpdate(ref ehit);
                    }
                }
            }
    ...
    public class SpaceMapDataStoreNode
    {
        ...
        public void TrySplit(int threshold)
        {
            ...
            List<MapDataStoreNode> newItems = new List<MapDataStoreNode>();

            Vector3 ncen = (BBMax + BBMin) * 0.5f;
            Vector3 next = (BBMax - BBMin) * 0.5f;
            float nsiz = Math.Max(next.X, next.Y);
            float nsizh = nsiz * 0.5f;

            foreach (MapDataStoreNode item in Items)
            {
                Vector3 imin = item.streamingExtentsMin;
                Vector3 imax = item.streamingExtentsMax;
                Vector3 icen = (imax + imin) * 0.5f;
                Vector3 iext = (imax - imin) * 0.5f;
                float isiz = Math.Max(iext.X, iext.Y);
    ...
    public class SpaceBoundsStoreNode
    {
        ...
        public void TrySplit(int threshold)
        {
            ...
            List<BoundsStoreItem> newItems = new List<BoundsStoreItem>();
            ...
            foreach (BoundsStoreItem item in Items)
            {
                Vector3 imin = item.Min;
                Vector3 imax = item.Max;
                Vector3 icen = (imax + imin) * 0.5f;
                Vector3 iext = (imax - imin) * 0.5f;
                float isiz = Math.Max(iext.X, iext.Y);

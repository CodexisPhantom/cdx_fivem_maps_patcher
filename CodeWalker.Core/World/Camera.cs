...
namespace CodeWalker.World
{
    public class Camera
    {
        ...
        private void UpdateFollow(float elapsed)
        {
            ...
            if (IsMapView)
            {
                ...
                if (FollowEntity != null) cpos = FollowEntity.Position;
        ...
        private void UpdateProjection() //CameraSpaceProjection& p, float mx, float my)
        {
            ...
            if (IsMapView || IsOrthographic) MouseRay.Position = MouseRayNear;
    ...
    public class Frustum
    {
        ...
        {
            Vector3 c = cen - Position;
            for (int i = 0; i < 5; i++)
            {
                Vector3 pn = Planes[i].Normal;
                float d = c.X * pn.X + c.Y * pn.Y + c.Z * pn.Z; //Vector3.Dot(c, pn);// 
                float r = e.X * (pn.X > 0 ? pn.X : -pn.X) + e.Y * (pn.Y > 0 ? pn.Y : -pn.Y) +
                          e.Z * (pn.Z > 0 ? pn.Z : -pn.Z); //Vector3.Dot(e, pn.Abs()); //
        ...
        {
            Vector3 c = (bmax + bmin) * 0.5f - Position;
            Vector3 e = (bmax - bmin) * 0.5f;
            for (int i = 0; i < 5; i++)
            {
                float pd = Planes[i].D;
                Vector3 pn = Planes[i].Normal;
                float d = c.X * pn.X + c.Y * pn.Y + c.Z * pn.Z;
                float r = e.X * (pn.X > 0 ? pn.X : -pn.X) + e.Y * (pn.Y > 0 ? pn.Y : -pn.Y) +
                          e.Z * (pn.Z > 0 ? pn.Z : -pn.Z);

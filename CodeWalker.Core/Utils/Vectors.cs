...
namespace CodeWalker
{
    ...
    public static class BoundingBoxMath
    {
        ...
        public static BoundingBox Transform(this BoundingBox b, Matrix mat)
        {
            ...
            Vector3 bbcenter = (b.Maximum + b.Minimum) * 0.5f;
            Vector3 bbextent = (b.Maximum - b.Minimum) * 0.5f;
    ...
    public static class BoundingCapsuleMath
    {
        ...
        {
            float dist = LineMath.PointSegmentDistance(ref sph.Center, ref capsule.PointA, ref capsule.PointB);
            float rads = sph.Radius + capsule.Radius;
            if (dist <= rads)
            {
    ...
    public static class LineMath
    {
        ...
        public static float DistanceFieldTest(in Vector2 s, in Vector2 p1, in Vector2 p2, out int winding)
        {
            ...
            float d = PointSegmentDistance(s, p1, p2);
            winding = 0;
            Vector2 d1 = p1 - s;
            Vector2 d2 = p2 - s;
        ...
        public static float PointSegmentDistance(in Vector2 v, in Vector2 a, in Vector2 b)
        {
            ...
            float t = FloatUtil.Saturate(Vector2.Dot(av, ab) / l2);
            Vector2 p = a + t * ab;
            return (p - v).Length();
        }
    }
    ...
    public static class TriangleMath
    {
        public static float AreaPart(ref Vector3 v1, ref Vector3 v2, ref Vector3 v3, out float angle)
        {
            Vector3 va = v2 - v1;
            Vector3 vb = v3 - v1;
            Vector3 na = Vector3.Normalize(va);
            Vector3 nb = Vector3.Normalize(vb);
            float a = va.Length();
            float b = vb.Length();
            double c = Math.Acos(Vector3.Dot(na, nb));
            float area = (float)(0.5 * a * b * Math.Sin(c));
        ...
        {
            float a1 = AreaPart(ref v1, ref v2, ref v3, out float t1);
            float a2 = AreaPart(ref v2, ref v3, ref v1, out float t2);
            float a3 = AreaPart(ref v3, ref v1, ref v2, out float t3);

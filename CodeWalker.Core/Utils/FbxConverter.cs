...
namespace CodeWalker
{
    public class FbxConverter
    {
        ...
        private Drawable TryConvertDrawable(FbxDocument fdoc, string name)
        {
            ...
            if (allVerts.Count > 0)
            {
                ...
                foreach (Vector3 vp in allVerts) bsRad = Math.Max(bsRad, (vp - bsCen).Length());
            }
        ...
        private FbxModel TryConvertModel(FbxNode mnode)
        {
            ...
            if (fnVerts == null || fnIndices == null) return null; //no mesh data.. abort!
            ...
            for (int i = 0; i < nNormals; i++)
            {
                ...
                if (!IsByPolygonVertexMapType(fnNorms)) continue;
                bool indexed = IsIndexToDirectRefType(fnNorms);
                if (indexed && aiNorms == null) continue; //need the index array if it's IndexToDirect!
            ...
            for (int i = 0; i < nBinormals; i++)
            {
                ...
                if (!IsByPolygonVertexMapType(fnBinorms)) continue;
                bool indexed = IsIndexToDirectRefType(fnBinorms);
                if (indexed && aiBinorms == null) continue; //need the index array if it's IndexToDirect!
            ...
            for (int i = 0; i < nTangents; i++)
            {
                ...
                if (!IsByPolygonVertexMapType(fnTangs)) continue;
                bool indexed = IsIndexToDirectRefType(fnTangs);
                if (indexed && aiTangs == null) continue; //need the index array if it's IndexToDirect!
            ...
            for (int i = 0; i < nTexcoords; i++)
            {
                ...
                if (!IsByPolygonVertexMapType(fnTexcs)) continue;
                bool indexed = IsIndexToDirectRefType(fnTexcs);
                if (indexed && aiTexcs == null) continue; //need the index array if it's IndexToDirect!
            ...
            for (int i = 0; i < nColours; i++)
            {
                ...
                if (!IsByPolygonVertexMapType(fnCols)) continue;
                bool indexed = IsIndexToDirectRefType(fnCols);
                if (indexed && aiCols == null) continue; //need the index array if it's IndexToDirect!
        ...
        private DrawableGeometry TryConvertGeometry(List<FbxPolygon> fPolys, FbxNode matNode, out AABB_s aabb)
        {
            ...
            for (int i = 0; i < vList.Count; i++)
            {
                ...
                int o = i * vStride;
                for (int j = 0; j < vStride; j++) vBytes[o + j] = v[j];
            }
        ...
        private ShaderFX TryConvertMaterial(FbxNode matNode)
        {
            ...
            if (texNames.Count > 1) spsName = "normal";
            ...
            switch (spsName)
            {
                ...
                case "default":
                    ...
                    AddShaderParam(pNames, pVals, ShaderParamNames.DiffuseSampler,
                        GetTextureBaseParam(texNames, 0)); //assume first texture is diffuse...
                ...
                case "normal":
                    ...
                    AddShaderParam(pNames, pVals, ShaderParamNames.DiffuseSampler,
                        GetTextureBaseParam(texNames, 0)); //assume first texture is diffuse...
                    AddShaderParam(pNames, pVals, ShaderParamNames.BumpSampler,
                        GetTextureBaseParam(texNames, 1)); //assume 2nd texture is normalmap..
                    ...
                    AddShaderParam(pNames, pVals, ShaderParamNames.specularFalloffMult,
                        new Vector4(20, 0, 0, 0)); //too metallic?
            ...
            for (int i = 0; i < pNames.Count; i++) nameHashes[i] = (MetaName)pNames[i];
        ...
        private TextureBase GetTextureBaseParam(List<string> texNames, int index)
        {
            ...
            if (texNames.Count > index)
            {
                ...
                if (nameval != null) name = texNames[index];
            }
        ...
        private void AddShaderParam(List<ShaderParamNames> paramNames, List<ShaderParameter> paramValues,
            ShaderParamNames paramName, object paramValue)
        {
        ...
        private bool IsByPolygonVertexMapType(FbxNode node)
        {
            ...
            if (mapType != "ByPolygonVertex") return false; //any other types?
    ...
    public class FbxVertex : IEquatable<FbxVertex>
    {
        ...
        private void WriteBytes<T>(T val, int offset) where T : struct
        {
            ...
            for (int i = 0; i < data.Length; i++) Bytes[offset + i] = data[i];
        }
        ...
        public override int GetHashCode()
        {
            ...
            if (Bytes != null)
                hashCode = hashCode * -1521134295 +
                           ((IStructuralEquatable)Bytes).GetHashCode(EqualityComparer<byte>.Default);
            return hashCode;
        }
    }
}
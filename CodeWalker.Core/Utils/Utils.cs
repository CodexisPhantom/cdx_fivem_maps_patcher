...
namespace CodeWalker
{
    public static class PathUtil
    {
        ...
        {
            string path = Path.Combine(AppPath, appRelativePath);
        ...
        {
            string path = GetFilePath(appRelativePath);
    ...
    public static class TextUtil
    {
        ...
        {
            if (bytes == null) return string.Empty; //file not found..

            if (bytes.Length > 3 && bytes[0] == 0xEF && bytes[1] == 0xBB && bytes[2] == 0xBF)
            {
                byte[] newb = new byte[bytes.Length - 3];
                for (int i = 3; i < bytes.Length; i++) newb[i - 3] = bytes[i];
    ...
    public static class FloatUtil
    {
        ...
        {
            if (float.TryParse(s, NumberStyles.Any, CultureInfo.InvariantCulture, out f)) return true;
            return false;
        }
        ...
        {
            return string.Format("x=\"{0}\" y=\"{1}\" z=\"{2}\" w=\"{3}\"", ToString(v.X), ToString(v.Y), ToString(v.Z),
                ToString(v.W));
        }

        public static string GetQuaternionXmlString(Quaternion q)
        {
            return string.Format("x=\"{0}\" y=\"{1}\" z=\"{2}\" w=\"{3}\"", ToString(q.X), ToString(q.Y), ToString(q.Z),
                ToString(q.W));
        }

        public static string GetHalf2String(Half2 v, string d = ", ")
        {
            float[] f = Half.ConvertToFloat(new[] { v.X, v.Y });
            return ToString(f[0]) + d + ToString(f[1]);
        }

        public static string GetHalf4String(Half4 v, string d = ", ")
        {
            float[] f = Half.ConvertToFloat(new[] { v.X, v.Y, v.Z, v.W });
        ...
        public static Vector2 ParseVector2String(string s)
        {
            ...
            string[] ss = s.Split(',');
            if (ss.Length > 0) TryParse(ss[0].Trim(), out p.X);
            if (ss.Length > 1) TryParse(ss[1].Trim(), out p.Y);
            return p;
        }
        ...
        public static Vector3 ParseVector3String(string s)
        {
            ...
            if (ss.Length > 0) TryParse(ss[0].Trim(), out p.X);
            if (ss.Length > 1) TryParse(ss[1].Trim(), out p.Y);
            if (ss.Length > 2) TryParse(ss[2].Trim(), out p.Z);
            return p;
        }
        ...
        public static Vector4 ParseVector4String(string s)
        {
            ...
            if (ss.Length > 1) TryParse(ss[1].Trim(), out p.Y);
            if (ss.Length > 2) TryParse(ss[2].Trim(), out p.Z);
            if (ss.Length > 3) TryParse(ss[3].Trim(), out p.W);
            return p;
        }

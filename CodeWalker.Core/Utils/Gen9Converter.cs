...
namespace CodeWalker.Core.Utils
{
    public class Gen9Converter
    {
        ...
        public void Convert()
        {
            ...
            if (outputFolder.EndsWith("\\") == false) outputFolder = outputFolder + "\\";
                ...
                foreach (string path in allpaths)
                    try
                    {
                            ...
                            foreach (RpfFile trpf in rpflist)
                            {
                                ...
                                foreach (RpfResourceFileEntry rfe in allentries)
                                {
                                    ...
                                    byte[]
                                        datain = trpf
                                            .ExtractFile(
                                                rfe); //unfortunately the data extracted here is decompressed but we need a compressed resource file to convert.
                                    datain = ResourceBuilder
                                        .Compress(datain); //not completely ideal to recompress it...
                                    ...
                                    RpfFile.CreateFile(dir, name, dataout);
                        ...
                        else
                        {
                            ...
                            if (dataout != null) File.WriteAllBytes(outpath, dataout);
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        public static byte[] TryConvert(byte[] data, string fileType, Action<string> log, string relpath,
            bool copyunconverted, out bool converted)
        {
            converted = false;
            ...
            switch (fileType)
            {
                ...
                case ".rpf":
                    log(
                        $"{relpath} - Cannot convert RPF files! Extract the contents of the RPF and convert that instead.");
                    return null;
                default:
                    if (copyunconverted)
                    {

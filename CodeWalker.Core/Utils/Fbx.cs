...
namespace CodeWalker
{
    ...
    public static class FbxIO
    {
        ...
        public static FbxDocument Read(byte[] data)
        {
            ...
            {
                bool isbinary = FbxBinary.IsBinary(stream);
                if (isbinary)
                {
                    FbxBinaryReader reader = new FbxBinaryReader(stream);
                    return reader.Read();
                }
                else //try ASCII
                {
                    FbxAsciiReader reader = new FbxAsciiReader(stream);
                    return reader.Read();
                }
            }
        }
    ...
    public class FbxAsciiReader
    {
        ...
        private Array ReadArray()
        {
            ...
            object token;
            ArrayType arrayType = ArrayType.Byte;
            long pos = 0;
        ...
        {
            FbxDocument ret = new FbxDocument();
    ...
    public class FbxAsciiWriter
    {
        ...
        private void BuildString(FbxNode node, StringBuilder sb, bool writeArrayLength, int indentLevel = 0)
        {
            ...
            for (int j = 0; j < node.Properties.Count; j++)
            {
                ...
                {
                    Array array = (Array)p;
                    Type elementType = p.GetType().GetElementType();
    ...
    public abstract class FbxBinary
    {
        ...
        {
            byte[] buf = new byte[headerString.Length];
        ...
        private static int GetTimestampVar(FbxNode timestamp, string element)
        {
            ...
            {
                object prop = elementNode.Properties[0];
        ...
        {
            FbxNode timestamp = document.GetRelative(timePath1 + "/" + timePath2);
            if (timestamp == null)
        ...
        {
            byte[] zeroes = new byte[Math.Max(footerZeroes1, footerZeroes2)];
        ...
        {
            byte[] buffer = new byte[Math.Max(footerZeroes1, footerZeroes2)];
    ...
    public class FbxBinaryReader : FbxBinary
    {
        ...
        private object ReadProperty()
        {
            ...
            switch (dataType)
            {
                ...
                case 'S':
                    ...
                    string str = len == 0 ? "" : Encoding.ASCII.GetString(stream.ReadBytes(len));
                    // Convert \0\1 to '::' and reverse the tokens
                    if (str.Contains(binarySeparator))
                    {
                        string[] tokens = str.Split(new[] { binarySeparator }, StringSplitOptions.None);
                        StringBuilder sb = new StringBuilder();
                        bool first = true;
        ...
        private Array ReadArray(ReadPrimitive readPrimitive, Type arrayType)
        {
            ...
            int compressedLen = stream.ReadInt32();
            Array ret = Array.CreateInstance(arrayType, len);
            BinaryReader s = stream;
            long endPos = stream.BaseStream.Position + compressedLen;
            if (encoding != 0)
            {
                if (errorLevel >= FbxErrorLevel.Checked)
                {
        ...
        public FbxNode ReadNode()
        {
            ...
            int numProperties = stream.ReadInt32();
            int propertyListLen = stream.ReadInt32();
            byte nameLen = stream.ReadByte();
            string name = nameLen == 0 ? "" : Encoding.ASCII.GetString(stream.ReadBytes(nameLen));
            ...
            long propertyEnd = stream.BaseStream.Position + propertyListLen;
            // Read properties
            ...
            // Read nested nodes
            long listLen = endOffset - stream.BaseStream.Position;
        ...
        public FbxDocument Read()
        {
            ...
            FbxDocument document = new FbxDocument { Version = (FbxVersion)stream.ReadInt32() };

            // Read nodes
            long dataPos = stream.BaseStream.Position;
            FbxNode nested;
            do
            {
                nested = ReadNode();
            ...
            bool validFooterExtension = CheckFooter(stream, document.Version);
    ...
    public class FbxBinaryWriter : FbxBinary
    {
        ...
        private static void WriteString(BinaryWriter stream, object obj)
        {
            ...
            {
                string[] tokens = str.Split(new[] { asciiSeparator }, StringSplitOptions.None);
                StringBuilder sb = new StringBuilder();
                bool first = true;
                for (int i = tokens.Length - 1; i >= 0; i--)
                {
        ...
        private void WriteArray(Array array, Type elementType, PropertyWriter writer)
        {
            ...
            long dataStart = stream.BaseStream.Position;
            if (compress)
            {
        ...
        private void WriteNode(FbxNode node)
        {
            ...
            else
            {
                ...
                long propertyLengthPos = stream.BaseStream.Position;
    ...
    public class FbxDocument : FbxNodeList
    {
        ...
        {
            FbxNode fobjs = this["Objects"];
            if (fobjs?.Nodes == null)
                return null;

            FbxNode fconns = this["Connections"];
            if (fconns?.Nodes == null)
                return null;

            Dictionary<long, FbxNode> fobjdict = new Dictionary<long, FbxNode>();
            List<FbxNode> rootnodes = new List<FbxNode>();

            foreach (FbxNode node in fobjs.Nodes) //put all the object nodes into a decktionary for the connections
            {
            ...
            foreach (FbxNode node in fconns.Nodes) //build the scene hierarchy by adding connections to object nodes
            {
                if (node == null) continue;
